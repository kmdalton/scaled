...
function loadforcelayout(misc_result) {
    
    //d3.json(filename, function(error, json) {
	
    root = JSON.parse(misc_result);

	for (var i = 0; i < root.nodes.length; i++) {
	    root.nodes[i].value = root.nodes[i].resnum;
	}

	for (var i = 0; i < root.links.length; i++) {
	    root.links[i].value = root.links[i].nogap;
	    //root.links[i].value = root.links[i].gap;
	}
