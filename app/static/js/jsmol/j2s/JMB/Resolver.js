...
Clazz.load (["J.api.JmolBioResolver"], "JMB.Resolver", ["java.lang.Boolean", "$.NullPointerException", "java.util.Arrays", "$.Hashtable", "JU.BS", "$.P3", "$.P4", "$.PT", "$.SB", "$.V3", "J.constant.EnumStructure", "JM.Group", "JMB.AlphaMonomer", "$.AlphaPolymer", "$.AminoMonomer", "$.AminoPolymer", "$.BioModel", "$.CarbohydrateMonomer", "$.CarbohydratePolymer", "$.Monomer", "$.NucleicMonomer", "$.NucleicPolymer", "$.PhosphorusMonomer", "$.PhosphorusPolymer", "JW.BSUtil", "$.Logger", "$.Measure", "$.Txt", "JV.JC"], function () {
...
this.vNorm = null;
this.plane = null;
if (!Clazz.isClassDefined ("JMB.Resolver.BondSorter")) {
JMB.Resolver.$Resolver$BondSorter$ ();
}
this.bsAssigned = null;
Clazz.instantialize (this, arguments);
}, JMB, "Resolver", null, J.api.JmolBioResolver);
Clazz.makeConstructor (c$, 
function () {
});
$_V(c$, "getBioModel", 
function (modelIndex, trajectoryBaseIndex, jmolData, modelProperties, modelAuxiliaryInfo) {
return  new JMB.BioModel (this.modelSet, modelIndex, trajectoryBaseIndex, jmolData, modelProperties, modelAuxiliaryInfo);
...
var m = null;
if ((distinguishingBits & 14) == 14) m = JMB.AminoMonomer.validateAndAllocate (chain, group3, seqcode, firstAtomIndex, lastAtomIndex, specialAtomIndexes, atoms);
 else if (distinguishingBits == 4) m = JMB.AlphaMonomer.validateAndAllocateA (chain, group3, seqcode, firstAtomIndex, lastAtomIndex, specialAtomIndexes);
 else if (((distinguishingBits & 8128) == 8128)) m = JMB.NucleicMonomer.validateAndAllocate (chain, group3, seqcode, firstAtomIndex, lastAtomIndex, specialAtomIndexes);
 else if (distinguishingBits == 8192) m = JMB.PhosphorusMonomer.validateAndAllocateP (chain, group3, seqcode, firstAtomIndex, lastAtomIndex, specialAtomIndexes);
 else if (JV.JC.checkCarbohydrate (group3)) m = JMB.CarbohydrateMonomer.validateAndAllocate (chain, group3, seqcode, firstAtomIndex, lastAtomIndex);
...
java.util.Arrays.sort (bondInfo, Clazz.innerTypeInstance (JMB.Resolver.BondSorter, this, null));
var t;
for (var i = 0; i < n; ) {
t = bondInfo[i];
var a1 = t[0];
...
JW.Measure.getPlaneThroughPoints (htAtoms.get (name1), htAtoms.get (name), htAtoms.get (name2), this.vNorm, this.vAB, this.vAC, this.plane);
var d = JW.Measure.distanceToPlane (this.plane, htAtoms.get (bondInfo[pt][1])) * factor;
...
bondInfo[i] = null;
continue;
}if (t != null) JW.Logger.info (" ligand " + group3 + ": " + bondInfo[i][0] + " - " + bondInfo[i][1] + " order " + bondInfo[i][2]);
}
return bondInfo;
...
JW.Logger.info ("Error adding H atoms to " + gName + g.getResno () + ": " + pts[i].length + " atoms should not be added to " + aName);
continue;
...
JW.Measure.getPlaneThroughPoints (atom1, atom, atom2, this.vNorm, this.vAB, this.vAC, this.plane);
d = JW.Measure.distanceToPlane (this.plane, pts[i][0]) * factor;
break;
}
if (pt < 0) {
JW.Logger.info ("Error adding H atoms to " + gName + g.getResno () + ": expected to only need 1 H but needed 2");
hName1 = hName2 = "H";
} else if (d < 0) {
...
this.modelSet.fixFormalCharges (JW.BSUtil.newBitSet2 (this.modelLoader.baseAtomIndex, this.modelLoader.modelSet.atomCount));
...
JW.Logger.info ("bond " + key + " was not used; order=" + value);
...
JW.Logger.info ("assigning order " + bonds[i].order + " to bond " + bonds[i]);
}
}, $fz.isPrivate = true, $fz));
$_M(c$, "setHydrogen", 
...
var group = groups[i];
var current;
if (!(Clazz.instanceOf (group, JMB.Monomer)) || (current = group).bioPolymer != null || previous != null && previous.getClass () !== current.getClass () || checkConnections && !current.isConnectedAfter (previous)) break;
previous = current;
count++;
}
...
if (Clazz.instanceOf (previous, JMB.AminoMonomer)) return  new JMB.AminoPolymer (monomers);
if (Clazz.instanceOf (previous, JMB.AlphaMonomer)) return  new JMB.AlphaPolymer (monomers);
if (Clazz.instanceOf (previous, JMB.NucleicMonomer)) return  new JMB.NucleicPolymer (monomers);
if (Clazz.instanceOf (previous, JMB.PhosphorusMonomer)) return  new JMB.PhosphorusPolymer (monomers);
if (Clazz.instanceOf (previous, JMB.CarbohydrateMonomer)) return  new JMB.CarbohydratePolymer (monomers);
JW.Logger.error ("Polymer.allocatePolymer() ... no matching polymer for monomor " + previous);
...
if (Clazz.instanceOf (models[i], JMB.BioModel)) (models[i]).addSecondaryStructure (type, structureID, serialID, strandCount, startChainID, startSeqCode, endChainID, endSeqCode, i0 + atomRange[0], i0 + atomRange[1], bsAssigned);
}
...
Clazz.instantialize (this, arguments);
}, JMB.Resolver, "BondSorter", null, java.util.Comparator);
$_V(c$, "compare", 
function (a, b) {
