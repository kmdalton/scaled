...
Clazz.load (["JM.Model"], "JMB.BioModel", ["java.lang.Float", "java.util.Hashtable", "JU.AU", "$.BS", "$.List", "$.SB", "J.api.Interface", "J.constant.EnumStructure", "JM.AtomCollection", "JMB.AlphaPolymer", "$.Monomer", "$.Resolver", "JW.BSUtil", "$.Escape", "$.Txt", "JV.Viewer"], function () {
c$ = Clazz.decorateAsClass (function () {
this.bioPolymerCount = 0;
this.bioPolymers = null;
Clazz.instantialize (this, arguments);
}, JMB, "BioModel", JM.Model);
Clazz.makeConstructor (c$, 
function (modelSet, modelIndex, trajectoryBaseIndex, jmolData, properties, auxiliaryInfo) {
Clazz.superConstructor (this, JMB.BioModel, [modelSet, modelIndex, trajectoryBaseIndex, jmolData, properties, auxiliaryInfo]);
...
for (var i = this.bioPolymerCount; --i >= 0; ) if (Clazz.instanceOf (this.bioPolymers[i], JMB.AlphaPolymer)) (this.bioPolymers[i]).addStructure (type, structureID, serialID, strandCount, startChainID, startSeqcode, endChainID, endSeqcode, istart, iend, bsAssigned);
...
if (!asDSSP || includeAlpha) for (var i = this.bioPolymerCount; --i >= 0; ) if (Clazz.instanceOf (this.bioPolymers[i], JMB.AlphaPolymer)) (this.bioPolymers[i]).calculateStructures (includeAlpha);
...
return (J.api.Interface.getOption ("dssx.DSSP")).calculateDssp (this.bioPolymers, this.bioPolymerCount, vHBonds, doReport, dsspIgnoreHydrogen, setStructure);
...
}if (bs2.nextSetBit (0) >= 0) sb.append ("select ").append (JW.Escape.eBS (bs2)).append (";backbone only;");
if (this.atomCount <= maxAtoms) return;
sb.append ("select ").append (JW.Escape.eBS (bs)).append (" & connected; wireframe only;");
if (bs !== this.bsAtoms) {
bs2.clearAll ();
bs2.or (this.bsAtoms);
bs2.andNot (bs);
if (bs2.nextSetBit (0) >= 0) sb.append ("select " + JW.Escape.eBS (bs2) + " & !connected;stars 0.5;");
}}, "JU.SB,~N");
$_V(c$, "fixIndices", 
...
var group = groups[i];
if (Clazz.instanceOf (group, JMB.Monomer)) {
var monomer = group;
...
if (!model.isBioModel || !(Clazz.instanceOf (g, JMB.Monomer))) continue;
var doCheck = checkConnections && !this.modelSet.isJmolDataFrameForModel (this.modelSet.atoms[g.firstAtomIndex].modelIndex);
var bp = ((g).getBioPolymer () == null ? JMB.Resolver.allocateBioPolymer (groups, i, doCheck) : null);
...
}, $fz.isPrivate = true, $fz), "JMB.BioPolymer");
...
iLastModel = iModel;
cmd.append ("  structure none ").append (JW.Escape.eBS (this.modelSet.getModelAtomBitSetIncludingDeleted (iModel, false))).append (comment).append (";\n");
}comment += " & (" + res1 + " - " + res2 + ")";
var stype = subtype.getBioStructureTypeName (false);
cmd.append ("  structure ").append (stype).append (" ").append (JW.Escape.eBS (bs)).append (comment).append (";\n");
} else {
var str;
var nx;
var sb;
...
case J.constant.EnumStructure.HELIX:
nx = ++nHelix;
if (sid == null || pdbFileMode) sid = JW.Txt.formatStringI ("%3N %3N", "N", nx);
...
sid = JW.Txt.formatStringI ("%3N %3A 0", "N", nx);
sid = JW.Txt.formatStringS (sid, "A", "S" + nx);
...
str = JW.Txt.formatStringS (str, "ID", sid);
str = JW.Txt.formatStringS (str, "GROUPA", group1);
str = JW.Txt.formatStringS (str, "CA", chain1);
str = JW.Txt.formatStringI (str, "RESA", res1);
str = JW.Txt.formatStringS (str, "GROUPB", group2);
str = JW.Txt.formatStringS (str, "CB", chain2);
str = JW.Txt.formatStringI (str, "RESB", res2);
sb.append (str);
...
for (var i = JMB.BioModel.pdbRecords.length; --i >= 0; ) {
var ichFound;
var strRecord = JMB.BioModel.pdbRecords[i];
