...
Clazz.load (["JM.BondCollection", "JU.BS", "$.List", "$.P3", "JW.BoxInfo"], "JM.ModelCollection", ["java.lang.Boolean", "$.Character", "$.Float", "java.util.Date", "$.Hashtable", "JU.AU", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "$.JmolModulationSet", "J.bspt.Bspf", "J.constant.EnumPalette", "$.EnumVdw", "J.io.XmlUtil", "JM.Atom", "$.AtomIteratorWithinModel", "$.AtomIteratorWithinModelSet", "$.Bond", "$.BondSet", "$.HBond", "$.LabelToken", "$.Model", "$.StateScript", "JW.BSUtil", "$.Edge", "$.Elements", "$.Escape", "$.JmolMolecule", "$.Logger", "$.Point3fi", "$.Txt", "JV.Viewer"], function () {
...
this.freezeModels ();
var ret = "";
var bsModels = JW.BSUtil.copyInvert (alreadyDefined, this.modelCount);
...
function (modelIndex) {
return (this.modelCount < 1 || modelIndex >= this.modelCount || modelIndex < 0 ? "" : JW.Escape.escapeModelFileNumber (this.modelFileNumbers[modelIndex]));
}, "~N");
$_M(c$, "getModelNumber", 
function (modelIndex) {
...
if (!isBiomodel) tokens = (leftJustify ? JM.LabelToken.compile (this.viewer, "HETATM%5.-5i %-4.4a%1AUNK %1c   1%1E   %8.3x%8.3y%8.3z" + occTemp, '\0', null) : JM.LabelToken.compile (this.viewer, "HETATM%5.-5i  %-3.3a%1AUNK %1c   1%1E   %8.3x%8.3y%8.3z" + occTemp, '\0', null));
 else if (isHetero) tokens = (leftJustify ? JM.LabelToken.compile (this.viewer, "HETATM%5.-5i %-4.4a%1A%3.-3n %1c%4.-4R%1E   %8.3x%8.3y%8.3z" + occTemp, '\0', null) : JM.LabelToken.compile (this.viewer, "HETATM%5.-5i  %-3.3a%1A%3.-3n %1c%4.-4R%1E   %8.3x%8.3y%8.3z" + occTemp, '\0', null));
 else tokens = (leftJustify ? JM.LabelToken.compile (this.viewer, "ATOM  %5.-5i %-4.4a%1A%3.-3n %1c%4.-4R%1E   %8.3x%8.3y%8.3z" + occTemp, '\0', null) : JM.LabelToken.compile (this.viewer, "ATOM  %5.-5i  %-3.3a%1A%3.-3n %1c%4.-4R%1E   %8.3x%8.3y%8.3z" + occTemp, '\0', null));
var XX = a.getElementSymbolIso (false).toUpperCase ();
out.append (JM.LabelToken.formatLabelAtomArray (this.viewer, a, tokens, '\0', null)).append (XX.length == 1 ? " " + XX : XX.substring (0, 2)).append ("  \n");
...
var a = this.atoms[i];
out.append (JM.LabelToken.formatLabelAtomArray (this.viewer, a, tokens, '\0', null));
...
this.molecules = JW.JmolMolecule.getMolecules (this.atoms, bsModelAtoms, biobranches, null);
...
var bsNew = JU.BS.newN (this.modelCount);
for (var i = this.atomCount; --i >= 0; ) {
var atom = this.atoms[i];
...
function (bsModels) {
var bs =  new JU.BS ();
if (bsModels == null && this.bsAll == null) this.bsAll = JW.BSUtil.setAll (this.atomCount);
...
if (bs == null) bs = this.bsAll = JW.BSUtil.setAll (this.atomCount);
...
return bs;
case 1095761926:
bs = JW.BSUtil.newBitSet2 (0, this.atomCount);
info = specInfo;
...
if (uc == null) {
JW.BSUtil.andNot (bs, this.models[i].bsAtoms);
continue;
...
return bs;
case 1089470478:
return JW.BSUtil.copy (this.bsSymmetry == null ? this.bsSymmetry = JU.BS.newN (this.atomCount) : this.bsSymmetry);
case 1614417948:
bs =  new JU.BS ();
...
bs = JW.BSUtil.andNot (bs, this.viewer.getDeletedAtoms ());
...
return X;
}, $fz.isPrivate = true, $fz), "JM.Atom,~N,JU.V3,JU.V3,~B");
...
sb.append (" name=").append (JU.PT.esc (this.getModelName (i))).append (" title=").append (JU.PT.esc (this.getModelTitle (i))).append (" hasVibrationVectors=\"").appendB (this.viewer.modelHasVibrationVectors (i)).append ("\" />");
}
sb.append ("\n</models>");
return sb.toString ();
});
...
this.viewer.setShapeSizeRD (0, this.viewer.getDefaultRadiusData (), JW.BSUtil.newAndSetBit (atomIndex));
...
if (nAtoms == 0) return "";
var xmlUtil = J.api.Interface.getOption ("io.XmlUtil");
...
var atom =  new JM.Atom ().setAtom (modelIndex, this.atomCount, xyz, radius, atomSymmetry, atomSite, atomicAndIsotopeNumber, formalCharge, isHetero);
this.models[modelIndex].atomCount++;
this.models[modelIndex].bsAtoms.set (this.atomCount);
if (JW.Elements.isElement (atomicAndIsotopeNumber, 1)) this.models[modelIndex].hydrogenCount++;
