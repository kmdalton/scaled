...
Clazz.load (null, "JM.LabelToken", ["java.lang.Character", "$.Float", "java.util.Hashtable", "JU.PT", "$.SB", "JM.Atom", "J.script.T"], function () {
...
ich = JM.LabelToken.setToken (viewer, strFormat, lt, cch, chAtom.charCodeAt (0), htValues);
}
...
if (strFormat == null || strFormat.length == 0) return null;
var tokens = JM.LabelToken.compile (viewer, strFormat, '\0', null);
return JM.LabelToken.formatLabelAtomArray (viewer, atom, tokens, '\0', null);
}, "JV.Viewer,JM.Atom,~S");
...
JM.LabelToken.appendAtomTokenValue (viewer, atom, t, strLabel, indices);
}}
return (strLabel == null ? null : strLabel.toString ().intern ());
}, "JV.Viewer,JM.Atom,~A,~S,~A");
...
JM.LabelToken.formatLabelAtomArray (viewer, bond.atom1, tokens, '1', indices);
JM.LabelToken.formatLabelAtomArray (viewer, bond.atom2, tokens, '2', indices);
return JM.LabelToken.getLabel (tokens);
}, "JV.Viewer,JM.Bond,~A,java.util.Map,~A");
...
var tokens = JM.LabelToken.compile (viewer, label, '\1', htValues);
if (tokens == null) return "";
JM.LabelToken.setValues (tokens, htValues);
var atoms = m.modelSet.atoms;
var indices = m.countPlusIndices;
for (var i = indices[0]; i >= 1; --i) if (indices[i] >= 0) JM.LabelToken.formatLabelAtomArray (viewer, atoms[indices[i]], tokens, String.fromCharCode (48 + i), null);
...
}, $fz.isPrivate = true, $fz), "JV.Viewer,~S,JM.LabelToken,~N,~N,java.util.Map");
...
break;
case 1766856708:
ptT = JM.Atom.atomPropertyTuple (atom, t.tok);
break;
case 135270408:
if (t.data != null) {
...
break;
case 1666189314:
floatT = JM.Atom.atomPropertyFloat (viewer, atom, t.tok);
break;
case 'r':
...
case 1641025539:
case 1238369286:
strT = JM.Atom.atomPropertyString (viewer, atom, t.tok);
break;
case 'W':
...
}, $fz.isPrivate = true, $fz), "JV.Viewer,JM.Atom,JM.LabelToken,JU.SB,~A");
$_M(c$, "format", 
