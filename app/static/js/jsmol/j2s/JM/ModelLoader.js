...
Clazz.load (["java.util.Hashtable", "JU.BS", "JV.JC"], "JM.ModelLoader", ["java.lang.Boolean", "$.Float", "$.NullPointerException", "java.util.Arrays", "JU.AU", "$.List", "$.P3", "$.PT", "$.SB", "$.V3", "J.api.Interface", "JM.Chain", "$.Group", "$.Model", "$.ModelSet", "JW.BSUtil", "$.Elements", "$.JmolMolecule", "$.Logger"], function () {
...
this.viewer = viewer;
this.modelSet =  new JM.ModelSet (viewer, modelSetName);
...
if (this.isPDB) {
this.jbr = J.api.Interface.getInterface ("JMB.Resolver");
...
if (nAtoms > 0) JW.Logger.info ("reading " + nAtoms + " atoms");
...
JW.Logger.info ("ModelSet: haveSymmetry:" + this.modelSet.someModelsHaveSymmetry + " haveUnitcells:" + this.someModelsHaveUnitcells + " haveFractionalCoord:" + this.modelSet.someModelsHaveFractionalCoordinates);
JW.Logger.info (this.adapterModelCount + " model" + (this.modelSet.modelCount == 1 ? "" : "s") + " in this collection. Use getProperty \"modelInfo\" or" + " getProperty \"auxiliaryInfo\" to inspect them.");
}var q = this.modelSet.getModelSetAuxiliaryInfoValue ("defaultOrientationQuaternion");
if (q != null) {
JW.Logger.info ("defaultOrientationQuaternion = " + q);
JW.Logger.info ("Use \"set autoLoadOrientation TRUE\" before loading or \"restore orientation DEFAULT\" after loading to view this orientation.");
...
JW.Logger.info ("creating " + key + " for model " + this.modelSet.getModelName (i));
...
JW.Logger.info (n + " trajectory steps read");
...
if (this.appendNew) {
this.modelSet.models[modelIndex] = (modelIsPDB ? this.jbr.getBioModel (modelIndex, trajectoryBaseIndex, jmolData, modelProperties, modelAuxiliaryInfo) :  new JM.Model (this.modelSet, modelIndex, trajectoryBaseIndex, jmolData, modelProperties, modelAuxiliaryInfo));
...
if (codes != null) {
JW.Logger.info ("model " + this.modelSet.getModelNumberDotted (i) + " alternative locations: " + codes);
}}
}, $fz.isPrivate = true, $fz), "~N");
...
var nRead = 0;
var models = this.modelSet.models;
if (this.modelSet.modelCount > 0) this.nullGroup =  new JM.Group ().setGroup ( new JM.Chain (this.modelSet.models[this.baseModelIndex], 32, -1), "", 0, -1, -1);
...
if (addH && JW.Elements.getElementNumber (isotope) == 1) this.jbr.setHaveHsAlready (true);
...
JW.Logger.info ("Default Van der Waals type for model" + " set to " + vdwtype.getVdwLabel ());
vdwtypeLast = vdwtype;
}}}
JW.Logger.info (nRead + " atoms created");
}, $fz.isPrivate = true, $fz), "J.api.JmolAdapter,~O");
$_M(c$, "getPdbCharge", 
...
}, $fz.isPrivate = true, $fz), "JM.Model,~N");
...
if (haveMultipleBonds && this.modelSet.someModelsHaveSymmetry && !this.viewer.getBoolean (603979794)) JW.Logger.info ("ModelSet: use \"set appletSymmetryToBonds TRUE \" to apply the file-based multiple bonds to symmetry-generated atoms.");
...
if (atom1 == null) {
JW.Logger.error ("bondAtoms cannot find atomUid1?:" + atomUid1);
return null;
}var atom2 = this.htAtomMap.get (atomUid2);
if (atom2 == null) {
JW.Logger.error ("bondAtoms cannot find atomUid2?:" + atomUid2);
return null;
...
JW.Logger.info ("ModelSet: autobonding; use  autobond=false  to not generate bonds automatically");
} else {
this.modelSet.initializeBspf ();
JW.Logger.info ("ModelSet: not autobonding; use  forceAutobond=true  to force automatic bond creation");
}}, $fz.isPrivate = true, $fz));
...
}, $fz.isPrivate = true, $fz), "~N,JM.Chain,~S,~N,~N,~N");
$_M(c$, "addGroup", 
...
}, $fz.isPrivate = true, $fz), "JM.Chain,JM.Group");
$_M(c$, "countGroup", 
...
if (ret.length > 0) JW.Logger.info (ret);
}, $fz.isPrivate = true, $fz));
...
if (n >= JW.Elements.elementNumberMax) n = JW.Elements.elementNumberMax + JW.Elements.altElementIndexFromNumber (n);
...
var bs = JW.JmolMolecule.getBranchBitSet (this.modelSet.atoms, b.atom2.index, bsToTest, null, b.atom1.index, false, true);
...
JM.ModelLoader.setBranch2dZ (this.modelSet.atoms[atomIndex], bs, bsToTest, v, v0, v1);
return bs;
...
}, $fz.isPrivate = true, $fz), "JM.Atom,JU.BS,JU.BS,JU.V3,JU.V3,JU.V3");
...
}, $fz.isPrivate = true, $fz), "JM.Atom,JM.Atom,JU.V3,JU.V3,JU.V3");
$_M(c$, "finalizeShapes", 
($fz = function () {
...
if (atomSetCollection == null) return null;
var adapter = viewer.getModelAdapter ();
var pt =  new JU.P3 ();
...
var i = -1;
var n = 0;
var loadAllData = (JW.BSUtil.cardinalityOf (bsSelected) == viewer.getAtomCount ());
...
if (i < 0) break;
n++;
if (JW.Logger.debugging) JW.Logger.debug ("atomIndex = " + i + ": " + atoms[i] + " --> (" + xyz.x + "," + xyz.y + "," + xyz.z);
...
bs.and (bsSelected);
if (loadAllData) {
n = JW.BSUtil.cardinalityOf (bs);
if (n == 0) {
JW.Logger.warn ("createAtomDataSet: no atom found at position " + pt);
continue;
} else if (n > 1 && JW.Logger.debugging) {
JW.Logger.debug ("createAtomDataSet: " + n + " atoms found at position " + pt);
}}switch (tokType) {
case 1146095631:
var vib = iterAtom.getVib ();
if (vib == null) continue;
if (JW.Logger.debugging) JW.Logger.debug ("xyz: " + pt + " vib: " + vib);
...
JW.Logger.info ("_vibrationName = " + vibName);
viewer.setStringProperty ("_vibrationName", vibName);
break;
case 1146095626:
JW.Logger.info (n + " atom positions read");
modelSet.recalculateLeadMidpointsAndWingVectors (-1);
if (n == modelSet.atomCount) return "boundbox {*};reset";
break;
}
return null;
