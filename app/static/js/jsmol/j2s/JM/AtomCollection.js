...
Clazz.load (["java.lang.Float", "JU.BS", "$.V3"], "JM.AtomCollection", ["java.lang.Character", "java.util.Arrays", "$.Hashtable", "JU.A4", "$.AU", "$.List", "$.M3", "$.P3", "$.PT", "J.api.Interface", "J.atomdata.RadiusData", "J.constant.EnumPalette", "$.EnumStructure", "$.EnumVdw", "JM.Group", "J.script.T", "JW.BSUtil", "$.Elements", "$.Escape", "$.Logger", "$.Measure", "$.Parser", "$.Txt", "$.Vibration", "JV.JC"], function () {
...
$_M(c$, "getElementName", 
function (i) {
return JW.Elements.elementNameFromNumber (this.atoms[i].getAtomicAndIsotopeNumber ());
}, "~N");
$_M(c$, "getAtomName", 
function (i) {
...
function (atom) {
return JW.Elements.getVanderwaalsMar (atom.getElementNumber (), J.constant.EnumVdw.JMOL) / 1000;
}, "JM.Atom");
...
if ((r = atom.getVanderwaalsRadiusFloat (this.viewer, J.constant.EnumVdw.AUTO)) > this.maxVanderwaalsRadius) this.maxVanderwaalsRadius = r;
}
});
...
var ec = (J.api.Interface.getOption ("geodesic.EnvelopeCalculation")).set (this.viewer, this.atomCount, null);
ec.calculate ( new J.atomdata.RadiusData (null, envelopeRadius, J.atomdata.RadiusData.EnumType.ABSOLUTE, null), 3.4028235E38, bsSelected, JW.BSUtil.copyInvert (bsSelected, this.atomCount), false, false, false, true);
var points = ec.getPoints ();
this.surfaceDistanceMax = 0;
this.bsSurface = ec.getBsSurfaceClone ();
this.surfaceDistance100s =  Clazz.newIntArray (this.atomCount, 0);
this.nSurfaceAtoms = JW.BSUtil.cardinalityOf (this.bsSurface);
...
if (d < 0 && JW.Logger.debugging) JW.Logger.debug ("draw d" + j + " " + JW.Escape.eP (points[j]) + " \"" + d + " ? " + atom.getInfo () + "\"");
...
break;
default:
JW.Logger.error ("unsettable atom property: " + J.script.T.nameOf (tok));
break;
}
}
...
return (v == null && forceNew ?  new JW.Vibration () : v);
}, "~N,~B");
...
if (Clazz.instanceOf (vib, JW.Vibration)) {
this.vibrations[atomIndex] = vib;
} else {
if (this.vibrations[atomIndex] == null) this.vibrations[atomIndex] =  new JW.Vibration ();
...
for (var i = 0; i < this.atoms.length; i++) this.hydrophobicities[i] = JW.Elements.getHydrophobicity (this.atoms[i].getGroupID ());
...
var lines = JW.Parser.markLines (dataString, ';');
var n = 0;
try {
...
JW.Logger.error ("AtomCollection.loadData error: " + e);
} else {
throw e;
}
}
...
var lines = JW.Parser.markLines (data, ';');
...
if (Clazz.exceptionOf (e, Exception)) {
JW.Logger.error ("Frame.loadCoordinate error: " + e);
} else {
throw e;
}
}
...
if (this.tainted[type] == null) this.tainted[type] = JU.BS.newN (this.atomCount);
JW.BSUtil.copy2 (bs, this.tainted[type]);
}, "JU.BS,~N");
$_M(c$, "unTaintAtoms", 
function (bs, type) {
if (this.tainted == null || this.tainted[type] == null) return;
...
}, $fz.isPrivate = true, $fz), "JM.Atom,J.atomdata.AtomData");
...
a.setFormalCharge (c);
this.taintAtom (i, 4);
if (JW.Logger.debugging) JW.Logger.debug ("atom " + a + " formal charge " + c0 + " -> " + c);
n++;
}}
return n;
}, "JU.BS");
...
var angle = JW.Measure.computeAngleABC (attached[i], atom, attached[j], true);
...
case 210:
case 300:
if (Math.abs (JW.Measure.computeTorsion (attached[0], atom, attached[1], attached[2], true)) > 162) return "trigonal planar";
...
}, $fz.isPrivate = true, $fz), "JM.Atom,~N,~B");
...
switch (tokType) {
case 1095761939:
for (i = this.atomCount; --i >= 0; ) if (this.atoms[i].getResno () == iSpec) bs.set (i);
...
break;
case 1048609:
return JW.BSUtil.copy (this.getChainBits (iSpec));
case 1048614:
return JW.BSUtil.copy (this.getSeqcodeBits (iSpec, true));
case 1613758470:
...
break;
case 1087373316:
bsInfo = JW.BSUtil.copy (specInfo);
...
if (i == 0) JW.Logger.error ("MISSING getAtomBits entry for " + J.script.T.nameOf (tokType));
return bs;
}, "~N,~O");
...
if (JW.Txt.isMatch (g3, name, checkStar, true)) {
...
return JW.Txt.isMatch (atom.getAtomName ().toUpperCase (), strPattern, checkStar, false);
}, $fz.isPrivate = true, $fz), "JM.Atom,~S,~B");
$_M(c$, "getSeqcodeBits", 
...
for (var i = this.atomCount; --i >= 0; ) {
var atom = this.atoms[i];
var d = JW.Measure.distanceToPlane (plane, atom);
...
if (bsInclude == null) bsInclude = JW.BSUtil.setAll (points.length);
for (var i = this.atomCount; --i >= 0; ) {
var atom = this.atoms[i];
...
if (this.bsModulated != null) JW.BSUtil.deleteBits (this.bsModulated, bsAtoms);
...
this.bsSurface = null;
this.surfaceDistance100s = null;
if (this.tainted != null) for (var i = 0; i < 14; i++) JW.BSUtil.deleteBits (this.tainted[i], bsAtoms);
...
{
if (JM.AtomCollection.userSettableValues.length != 14) JW.Logger.error ("AtomCollection.java userSettableValues is not length TAINT_MAX!");
