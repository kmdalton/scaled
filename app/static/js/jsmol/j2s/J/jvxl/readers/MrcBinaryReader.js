...
Clazz.load (["J.jvxl.readers.MapFileReader"], "J.jvxl.readers.MrcBinaryReader", ["java.lang.Exception", "$.Float", "JU.SB", "JW.Logger"], function () {
...
JW.Logger.info ("nx=" + this.nx + " not displayable as MRC file");
throw  new Exception ("MRC file type not readable");
}JW.Logger.info ("reading nonstandard little-endian MRC file");
...
}JW.Logger.info ("MRC header: mode: " + this.mode);
JW.Logger.info ("MRC header: nx ny nz: " + this.nx + " " + this.ny + " " + this.nz);
this.nxyzStart[0] = this.binarydoc.readInt ();
this.nxyzStart[1] = this.binarydoc.readInt ();
this.nxyzStart[2] = this.binarydoc.readInt ();
JW.Logger.info ("MRC header: nxyzStart: " + this.nxyzStart[0] + " " + this.nxyzStart[1] + " " + this.nxyzStart[2]);
...
JW.Logger.info ("MRC header: na nb nc: " + this.na + " " + this.nb + " " + this.nc);
...
JW.Logger.info ("MRC header: mapc mapr maps: " + this.mapc + " " + this.mapr + " " + this.maps);
...
JW.Logger.info ("MRC header: dmin,dmax,dmean: " + this.dmin + "," + this.dmax + "," + this.dmean);
ispg = this.binarydoc.readInt ();
nsymbt = this.binarydoc.readInt ();
JW.Logger.info ("MRC header: ispg,nsymbt: " + ispg + "," + nsymbt);
...
JW.Logger.info ("MRC header: origin: " + this.origin);
...
JW.Logger.info ("MRC header: rms: " + rmsDeviation);
nlabel = this.binarydoc.readInt ();
JW.Logger.info ("MRC header: labels: " + nlabel);
...
if (i < nlabel) {
this.labels[i] = s;
JW.Logger.info (this.labels[i]);
}}
for (var i = 0; i < nsymbt; i += 80) {
var position = this.binarydoc.getPosition ();
var s = this.binarydoc.readString (80).trim ();
if (s.indexOf ('\0') != s.lastIndexOf ('\0')) {
JW.Logger.error ("File indicates " + nsymbt + " symmetry lines, but " + i + " found!");
this.binarydoc.seek (position);
break;
}JW.Logger.info ("MRC file symmetry information: " + s);
}
JW.Logger.info ("MRC header: bytes read: " + this.binarydoc.getPosition () + "\n");
...
JW.Logger.info ("Cutoff set to (mean + rmsDeviation*" + sigma + " = " + this.params.cutoff + ")\n");
