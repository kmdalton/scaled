...
Clazz.load (["J.jvxl.readers.AtomDataReader", "JU.P3", "$.P4", "$.V3"], "J.jvxl.readers.IsoSolventReader", ["java.lang.Float", "java.util.Hashtable", "JU.BS", "$.List", "$.P3i", "J.jvxl.data.MeshData", "JW.BSUtil", "$.Logger", "$.Measure", "$.MeshSurface", "$.TempArray"], function () {
...
}}if (this.thisAtomSet == null) this.thisAtomSet = JW.BSUtil.setAll (this.myAtomCount);
}, "~B");
$_V(c$, "generateCube", 
function () {
...
var fraction = fReturn[0] = JW.MeshSurface.getSphericalInterpolationFraction ((this.voxelSource[vA] < 0 ? this.solventRadius : this.atomRadius[this.voxelSource[vA] - 1]), valueA, valueB, edgeVector.length ());
...
this.meshDataServer.fillMeshData (this.meshData, 3, null);
this.meshData =  new J.jvxl.data.MeshData ();
}}if (this.params.thePlane != null && this.params.slabInfo == null) this.params.addSlabInfo (JW.TempArray.getSlabWithinRange (-100, 0));
});
...
JW.Logger.info ("cavities include " + n + " voxel points");
...
this.thisAtomSet = JW.BSUtil.setAll (this.myAtomCount);
...
this.getEdges ();
JW.Logger.info (this.vEdges.size () + " edges");
this.vFaces =  new JU.List ();
this.getFaces ();
JW.Logger.info (this.vFaces.size () + " faces");
this.bsLocale = null;
this.htEdges = null;
this.iter.release ();
this.iter = null;
...
this.noFaceSpheres = JW.BSUtil.setAll (this.myAtomCount);
...
var dCT = JW.Measure.distanceToPlane (this.plane, ptC);
...
if (JW.Measure.isInTetrahedron (this.ptXyzTemp, ptA, ptB, ptC, ptS, this.plane, this.vTemp, this.vTemp2, this.vTemp3, false)) {
...
JW.Measure.getPlaneThroughPoint (this.p, this.vTemp, this.plane);
