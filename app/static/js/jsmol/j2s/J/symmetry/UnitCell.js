...
Clazz.load (["JW.SimpleUnitCell", "JU.P3", "JV.JC"], "J.symmetry.UnitCell", ["java.lang.Float", "JU.M4", "$.V3", "J.api.Interface", "JW.BoxInfo", "$.Escape"], function () {
...
}, J.symmetry, "UnitCell", JW.SimpleUnitCell);
...
}return t.setFromThermalEquation (Bcart, JW.Escape.eAF (parBorU));
}, "~A");
...
cell0 =  new JU.P3 ();
cell1 =  new JU.P3 ();
JW.SimpleUnitCell.ijkToPoint3f (Clazz.floatToInt (this.unitCellMultiplier.x), cell0, 0);
JW.SimpleUnitCell.ijkToPoint3f (Clazz.floatToInt (this.unitCellMultiplier.y), cell1, 0);
cell1.sub (cell0);
}for (var i = 0; i < 8; i++) {
var pt = pts[i] = JU.P3.newP (JW.BoxInfo.unitCubePoints[i]);
if (cell0 != null) {
...
return JW.BoxInfo.getCanonicalCopy (pts, scale);
}, "~N,~B");
...
this.matrixFractionalToCartesian.rotTrans2 (JW.BoxInfo.unitCubePoints[i], this.vertices[i]);
}
...
function () {
var s = "";
if (this.fractionalOffset != null) s += "  unitcell " + JW.Escape.eP (this.fractionalOffset) + ";\n";
if (this.unitCellMultiplier != null) s += "  unitcell " + JW.Escape.eP (this.unitCellMultiplier) + ";\n";
return s;
});
