...
Clazz.load (["JU.P3", "$.V3"], "J.symmetry.PointGroup", ["java.lang.Float", "java.util.Hashtable", "JU.List", "$.Quat", "$.SB", "JW.BSUtil", "$.Escape", "$.Logger", "$.Txt"], function () {
...
JW.Logger.error ("Too many atoms for point group calculation");
...
var atomCount = JW.BSUtil.cardinalityOf (bsAtoms);
...
}, $fz.isPrivate = true, $fz), "JU.Quat,JU.P3,~N");
$_M(c$, "isLinear", 
($fz = function (atoms) {
var v1 = null;
...
$_M(c$, "getPlane", 
($fz = function (v3) {
if (!this.haveAxis (0, v3) && this.checkOperation (JU.Quat.newVA (v3, 180), this.center, -1)) this.axes[0][this.nAxes[0]++] = Clazz.innerTypeInstance (J.symmetry.PointGroup.Operation, this, null, v3);
return this.nAxes[0];
}, $fz.isPrivate = true, $fz), "JU.V3");
...
if (!haveType || type.equalsIgnoreCase ("Ci")) sb.append ("draw pg0").append (m).append (this.haveInversionCenter ? "inv " : " ").append (JW.Escape.eP (this.center)).append (this.haveInversionCenter ? "\"i\";\n" : ";\n");
var offset = 0.1;
...
sb.append ("draw pgva").append (m).append (label).append ("_").appendI (j + 1).append (" width 0.05 scale ").appendF (scale).append (" ").append (JW.Escape.eP (v));
v.scaleAdd2 (-2, op.normalOrAxis, v);
var isPA = (this.principalAxis != null && op.index == this.principalAxis.index);
sb.append (JW.Escape.eP (v)).append ("\"").append (label).append (isPA ? "*" : "").append ("\" color ").append (isPA ? "red" : op.type == 2 ? "blue" : "yellow").append (";\n");
}
}
if (!haveType || type.equalsIgnoreCase ("Cs")) for (var j = 0; j < this.nAxes[0]; j++) {
if (index > 0 && j + 1 != index) continue;
op = this.axes[0][j];
sb.append ("draw pgvp").append (m).appendI (j + 1).append ("disk scale ").appendF (scaleFactor * this.radius * 2).append (" CIRCLE PLANE ").append (JW.Escape.eP (this.center));
v.add2 (op.normalOrAxis, this.center);
sb.append (JW.Escape.eP (v)).append (" color translucent yellow;\n");
v.add2 (op.normalOrAxis, this.center);
sb.append ("draw pgvp").append (m).appendI (j + 1).append ("ring width 0.05 scale ").appendF (scaleFactor * this.radius * 2).append (" arc ").append (JW.Escape.eP (v));
v.scaleAdd2 (-2, op.normalOrAxis, v);
sb.append (JW.Escape.eP (v));
v.add3 (0.011, 0.012, 0.013);
sb.append (JW.Escape.eP (v)).append ("{0 360 0.5} color ").append (this.principalPlane != null && op.index == this.principalPlane.index ? "red" : "blue").append (";\n");
}
...
if (this.info == null) sb.append ("\n\n").append (this.name).append ("\t").append (ctype).append ("\t").append (JW.Escape.eP (this.center));
...
JW.Txt.rightJustify (sb, "    ", this.nAxes[0] + "\t");
JW.Txt.rightJustify (sb, "    ", this.nAxes[0] + "\n");
sb.append (this.name).append ("\tCn\t");
JW.Txt.rightJustify (sb, "    ", nType[1][0] + "\t");
JW.Txt.rightJustify (sb, "    ", nType[1][1] + "\n");
sb.append (this.name).append ("\tSn\t");
JW.Txt.rightJustify (sb, "    ", nType[2][0] + "\t");
JW.Txt.rightJustify (sb, "    ", nType[2][1] + "\n");
sb.append (this.name).append ("\t\tTOTAL\t");
JW.Txt.rightJustify (sb, "    ", nTotal + "\n");
...
this.type = 3;
this.order = 1;
if (JW.Logger.debugging) JW.Logger.debug ("new operation -- " + J.symmetry.PointGroup.typeNames[this.type]);
});
Clazz.makeConstructor (c$, 
function (a, b) {
this.index = ++this.b$["J.symmetry.PointGroup"].nOps;
...
if (JW.Logger.debugging) JW.Logger.debug ("new operation -- " + (this.order == b ? "S" : "C") + this.order + " " + this.normalOrAxis);
}, "JU.V3,~N");
Clazz.makeConstructor (c$, 
function (a) {
if (a == null) return;
...
this.type = 0;
this.normalOrAxis = JU.Quat.newVA (a, 180).getNormal ();
if (JW.Logger.debugging) JW.Logger.debug ("new operation -- plane " + this.normalOrAxis);
}, "JU.V3");
$_M(c$, "getLabel", 
function () {
switch (this.type) {
