...
Clazz.load (["JU.M4"], "J.symmetry.SymmetryOperation", ["java.lang.Float", "JU.List", "$.Matrix", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "JW.Escape", "$.Logger", "$.Measure", "$.Parser"], function () {
...
this.xyz = xyz;
JW.Parser.parseStringInfestedFloatArray (xyz, null, this.linearRotTrans);
...
if (JW.Logger.debugging) JW.Logger.debug ("" + this);
return true;
}, "~S,~N,~B");
$_M(c$, "setMatrix", 
($fz = function (isReverse) {
...
strT = "";
if (rowPt++ > 2 && modDim == 0) {
JW.Logger.warn ("Symmetry Operation? " + xyz);
return null;
}break;
case '.':
isDecimal = true;
...
JW.Logger.warn ("symmetry character?" + ch);
}}
...
info = JW.Measure.computeHelicalAxis (null, 135266306, pt00, p0, JU.Quat.getQuaternionFrame (p0, p1, p2).div (JU.Quat.getQuaternionFrame (pt00, pt01, pt02)));
var pa1 = info[0];
var ax1 = info[1];
...
if (haveinversion) {
pt1.add2 (pa1, vtemp);
ang2 = Math.round (JW.Measure.computeTorsion (ptinv, pa1, pt1, p0, true));
} else if (pitch1 == 0) {
pt1.setT (pa1);
ptemp.scaleAdd2 (1, pt1, vtemp);
ang2 = Math.round (JW.Measure.computeTorsion (pt00, pa1, ptemp, p0, true));
} else {
...
draw1.append (drawid).append ("rotVector2 diameter 0.1 ").append (JW.Escape.eP (pa1)).append (JW.Escape.eP (ptemp)).append (" color red");
}scale = p0.distance (pt1);
draw1.append (drawid).append ("rotLine1 ").append (JW.Escape.eP (pt1)).append (JW.Escape.eP (ptinv)).append (" color red");
draw1.append (drawid).append ("rotLine2 ").append (JW.Escape.eP (pt1)).append (JW.Escape.eP (p0)).append (" color red");
} else if (pitch1 == 0) {
var isSpecial = (pt00.distance (p0) < 0.2);
if (!isSpecial) {
draw1.append (drawid).append ("rotLine1 ").append (JW.Escape.eP (pt00)).append (JW.Escape.eP (pa1)).append (" color red");
draw1.append (drawid).append ("rotLine2 ").append (JW.Escape.eP (p0)).append (JW.Escape.eP (pa1)).append (" color red");
}vtemp.scale (3);
ptemp.scaleAdd2 (-1, vtemp, pa1);
draw1.append (drawid).append ("rotVector2 diameter 0.1 ").append (JW.Escape.eP (pa1)).append (JW.Escape.eP (ptemp)).append (" color red");
pt1.setT (pa1);
...
draw1.append (drawid).append ("rotLine1 ").append (JW.Escape.eP (pt00)).append (JW.Escape.eP (pa1)).append (" color red");
ptemp.add2 (pa1, vtemp);
draw1.append (drawid).append ("rotLine2 ").append (JW.Escape.eP (p0)).append (JW.Escape.eP (ptemp)).append (" color red");
pt1.scaleAdd2 (0.5, vtemp, pa1);
}ptemp.add2 (pt1, vtemp);
if (haveinversion && pitch1 != 0) {
draw1.append (drawid).append ("rotRotLine1").append (JW.Escape.eP (pt1)).append (JW.Escape.eP (ptinv)).append (" color red");
draw1.append (drawid).append ("rotRotLine2").append (JW.Escape.eP (pt1)).append (JW.Escape.eP (p0)).append (" color red");
}draw1.append (drawid).append ("rotRotArrow arrow width 0.10 scale " + scale + " arc ").append (JW.Escape.eP (pt1)).append (JW.Escape.eP (ptemp));
ptemp.setT (haveinversion ? ptinv : pt00);
if (ptemp.distance (p0) < 0.1) ptemp.set (Math.random (), Math.random (), Math.random ());
draw1.append (JW.Escape.eP (ptemp));
ptemp.set (0, ang, 0);
draw1.append (JW.Escape.eP (ptemp)).append (" color red");
draw1.append (drawid).append ("rotVector1 vector diameter 0.1 ").append (JW.Escape.eP (pa1)).append (JW.Escape.eP (vtemp)).append ("color ").append (color);
}if (ismirrorplane) {
if (pt00.distance (pt0) > 0.2) draw1.append (drawid).append ("planeVector arrow ").append (JW.Escape.eP (pt00)).append (JW.Escape.eP (pt0)).append (" color indigo");
if (trans != null) {
...
var pts = v.get (i);
draw1.append (drawid).append ("planep").appendI (i).append (" ").append (JW.Escape.eP (pts[0])).append (JW.Escape.eP (pts[1]));
if (pts.length == 3) draw1.append (JW.Escape.eP (pts[2]));
draw1.append (" color translucent ").append (color);
}
if (v.size () == 0) {
ptemp.add2 (pa1, ax1);
draw1.append (drawid).append ("planeCircle scale 2.0 circle ").append (JW.Escape.eP (pa1)).append (JW.Escape.eP (ptemp)).append (" color translucent ").append (color).append (" mesh fill");
}}if (haveinversion) {
draw1.append (drawid).append ("invPoint diameter 0.4 ").append (JW.Escape.eP (ipt));
draw1.append (drawid).append ("invArrow arrow ").append (JW.Escape.eP (pt00)).append (JW.Escape.eP (ptinv)).append (" color indigo");
if (!isinversion) {
...
draw1.append (drawid).append ("transVector vector ").append (JW.Escape.eP (pt0)).append (JW.Escape.eP (trans));
}draw1.append ("\nvar pt00 = " + JW.Escape.eP (pt00));
draw1.append ("\nvar p0 = " + JW.Escape.eP (p0));
...
draw1.append (drawid).append ("offsetFrameX diameter 0.20 @{set2.xyz} @{set2.xyz + ").append (JW.Escape.eP (v01)).append ("*0.9} color red");
draw1.append (drawid).append ("offsetFrameY diameter 0.20 @{set2.xyz} @{set2.xyz + ").append (JW.Escape.eP (v02)).append ("*0.9} color green");
draw1.append (drawid).append ("offsetFrameZ diameter 0.20 @{set2.xyz} @{set2.xyz + ").append (JW.Escape.eP (v03)).append ("*0.9} color purple");
draw1.append ("\n}}\n");
cmds = draw1.toString ();
draw1 = null;
drawid = null;
...
s.append (id).append (" diameter ").appendF (diameter).append (JW.Escape.eP (pt0)).append (JW.Escape.eP (pt1)).append (" color ").append (color);
