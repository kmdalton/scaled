...
Clazz.load (["J.script.ScriptParam"], "J.script.ScriptExpr", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "$.Map", "JU.BS", "$.CU", "$.List", "$.M34", "$.M4", "$.P3", "$.P4", "$.PT", "$.SB", "J.api.Interface", "JM.Atom", "$.BondSet", "$.Group", "$.ModelCollection", "J.script.SV", "$.ScriptContext", "$.ScriptMathProcessor", "$.T", "JW.BSUtil", "$.Elements", "$.Escape", "$.Logger", "$.Measure", "$.Txt"], function () {
c$ = Clazz.decorateAsClass (function () {
this.debugHigh = false;
this.cmdExt = null;
this.tempStatement = null;
Clazz.instantialize (this, arguments);
}, J.script, "ScriptExpr", J.script.ScriptParam);
$_M(c$, "getCmdExt", 
function () {
return (this.cmdExt == null ? (this.cmdExt = J.api.Interface.getOption ("scriptext.CmdExt")).init (this) : this.cmdExt);
});
$_V(c$, "parameterExpressionList", 
...
function (pt, ptMax, key, ignoreComma, asVector, ptAtom, isArrayItem, localVars, localVar, isAssignment) {
var v;
var res;
...
var nParen = 0;
var topLevel = true;
var rpn =  new J.script.ScriptMathProcessor (this, isAssignment, isArrayItem, asVector, false, false);
if (ptMax < pt) ptMax = this.slen;
var ptEq = (isAssignment ? 0 : 1);
...
if (isAssignment && nSquare == 1 && this.tokAt (i + 1) == 269484436) isAssignment = rpn.endAssignment ();
}
switch (tok) {
case 1060866:
...
var bsX =  new JU.BS ();
var sout = (isFor ?  new Array (JW.BSUtil.cardinalityOf (bsAtoms)) : null);
if (localVars == null) localVars =  new java.util.Hashtable ();
bsX.set (0);
...
case 6:
case 14:
rpn.addX (J.script.SV.newT (this.theToken));
break;
case 1048582:
...
rpn.addX (J.script.SV.newV (8, ptc));
} catch (e) {
...
case 1048584:
case 1048583:
if (isAssignment && topLevel && this.tokAt (i + 2) == 269484436) isAssignment = rpn.endAssignment ();
if (ptEq == 0 && topLevel) {
...
default:
rpn.addOp (J.script.T.o (269484096, "["));
rpn.addXStr (this.optParameterAsString (++i));
rpn.addOp (J.script.T.o (269484097, "]"));
continue;
}
...
$var = this.viewer.getOrSetNewVariable (name, false);
switch ($var.tok) {
case 2:
case 3:
...
this.st = code;
}var rpn =  new J.script.ScriptMathProcessor (this, false, false, false, mustBeBitSet, allowUnderflow);
var val;
...
break;
case 135267841:
rpn.addX (J.script.SV.newT (instruction));
rpn.addX (J.script.SV.newV (9, this.hklParameter (pc + 2)));
pc = this.iToken;
break;
case 135266319:
rpn.addX (J.script.SV.newT (instruction));
rpn.addX (J.script.SV.newV (9, this.planeParameter (pc + 2)));
pc = this.iToken;
break;
case 1048581:
rpn.addX (J.script.SV.newT (instruction));
...
break;
case 1114638363:
rpn.addXBs (JW.BSUtil.copy (this.viewer.getSelectedAtoms ()));
break;
case 3145770:
rpn.addXBs (JW.BSUtil.copy (this.viewer.getHiddenSet ()));
break;
case 1060869:
rpn.addXBs (JW.BSUtil.copy (this.viewer.getMotionFixedAtoms ()));
break;
case 3145768:
rpn.addXBs (JW.BSUtil.copyInvert (this.viewer.getHiddenSet (), atomCount));
break;
case 3145776:
...
if (isInMath) rpn.addXNum (instruction);
 else rpn.addXBs (this.getAtomBits (1048614, Integer.$valueOf (J.script.ScriptExpr.getSeqCode (instruction))));
break;
case 1048615:
if (isInMath) {
...
rpn.addXBs (this.getAtomBits (1048615, [J.script.ScriptExpr.getSeqCode (instruction), J.script.ScriptExpr.getSeqCode (code[++pc]), chainID]));
if (chainID != -1) pc += 2;
break;
case 1095761926:
case 1095761925:
...
if (J.script.T.tokAttr (tokValue, 1073741824)) {
if ("_modelNumber".equalsIgnoreCase (val)) {
var modelIndex = this.viewer.getCurrentModelIndex ();
val = Integer.$valueOf (modelIndex < 0 ? 0 : this.viewer.getModelFileNumber (modelIndex));
} else {
val = J.script.SV.nValue (this.getParameter (val, 1073742190));
...
if (tokWhat == 1641025539 || tokWhat == 1238369286 || tokWhat == 1087375365) isStringProperty = !(isIntProperty = (comparisonValue != 2147483647));
 ...
 else if (Clazz.instanceOf (val, Float) && isModel) comparisonValue = JM.ModelCollection.modelFileNumberFromFloat ((val).floatValue ());
...
if (!this.isBondSet && this.viewer.excludeAtoms (bs, ignoreSubset).length () > this.viewer.getAtomCount ()) bs.clearAll ();
...
function (bs) {
var l =  new JU.List ();
l.addLast (J.script.SV.newV (10, bs));
return l;
}, "JU.BS");
...
switch (tokWhat) {
default:
propertyFloat = JM.Atom.atomPropertyFloat (this.viewer, atom, tokWhat);
break;
case 1716520985:
...
for (var i = atomCount; --i >= 0; ) {
var propertyString = JM.Atom.atomPropertyString (this.viewer, atoms[i], tokWhat);
...
case 269484436:
case 269484438:
return (JW.Txt.isMatch (propertyValue, comparisonValue, true, true) == (tokOperator == 269484436));
default:
this.invArg ();
}
return false;
...
switch (tokOperator) {
case 269484435:
return JW.BSUtil.newBitSet2 (0, ival);
case 269484434:
return JW.BSUtil.newBitSet2 (0, ival + 1);
case 269484433:
return JW.BSUtil.newBitSet2 (ival, atomCount);
case 269484432:
return JW.BSUtil.newBitSet2 (ival + 1, atomCount);
case 269484436:
return (ival < atomCount ? JW.BSUtil.newBitSet2 (ival, ival + 1) :  new JU.BS ());
case 269484438:
default:
bs = JW.BSUtil.setAll (atomCount);
...
break;
case 1095766024:
return JW.BSUtil.copy (this.viewer.getConformation (-1, ival - 1, false));
case 1297090050:
...
}var bs1 = JW.BSUtil.copy (propertyBitSet);
...
if (mustBeSettable && isExpression && !J.script.T.tokAttr (tok, 2048)) return null;
return J.script.SV.newSV (269484241, tok, this.paramAsStr (i));
}, "~N,~B,~B");
...
bsNew = (tok == 1141899265 ? (isAtoms ? bs : this.viewer.getAtomBits (1678770178, bs)) : (isAtoms ?  new JM.BondSet (this.viewer.getBondsForSelectedAtoms (bs)) : bs));
var i;
switch (minmaxtype) {
case 32:
...
case 135368713:
bsAtom.set (i);
fv = J.script.SV.fValue (this.getUserFunctionResult (userFunction, params, tokenAtom));
bsAtom.clear (i);
break;
case 1716520985:
...
break;
case 1276118018:
if (planeRef != null) fv = JW.Measure.distanceToPlane (planeRef, atom);
 else fv = atom.distance (ptRef);
break;
default:
fv = JM.Atom.atomPropertyFloat (this.viewer, atom, tok);
}
...
break;
case 2:
var s = JM.Atom.atomPropertyString (this.viewer, atom, tok);
switch (minmaxtype) {
case 256:
...
break;
case 3:
var t = JM.Atom.atomPropertyTuple (atom, tok);
...
for (var i = len; --i >= 0; ) {
var v = vout.get (i);
if (Clazz.instanceOf (v, JU.P3)) sout[i] = JW.Escape.eP (v);
...
($fz = function (m) {
var bs = JU.BS.newN (this.viewer.getAtomCount ());
if (this.chk) return bs;
...
var tv = J.script.SV.newS ("").setv (v.get (nv - 1));
if (nv > 1) {
...
var f = tv.asFloat ();
switch (J.script.T.getTokFromName (sel.asString ())) {
case 1112541205:
p.x = f;
break;
case 1112541206:
...
var nAtoms = this.viewer.getAtomCount ();
var nbs = bs.cardinality ();
if (propertyName.startsWith ("property_")) {
var obj = (tv.tok == 7 ? J.script.SV.flistValue (tv, tv.getList ().size () == nbs ? nbs : nAtoms) : tv.asString ());
this.viewer.setData (propertyName, [propertyName, obj, JW.BSUtil.copy (bs), Integer.$valueOf (tv.tok == 7 ? 1 : 0)], nAtoms, 0, 0, tv.tok == 7 ? 2147483647 : -2147483648, 0);
break;
}this.setBitsetProperty (bs, J.script.T.getTokFromName (propertyName), tv.asInt (), tv.asFloat (), tv);
}
...
if (needVariable) {
if (key.startsWith ("_") || (t = this.viewer.getOrSetNewVariable (key, true)) == null) this.errorStr (22, key);
}if (t != null) {
t.setv (tv);
t.setModified (true);
return t;
...
this.viewer.setData (key, [key, "" + vv, JW.BSUtil.copy (this.viewer.getSelectedAtoms ()), Integer.$valueOf (0)], this.viewer.getAtomCount (), 0, 0, -2147483648, 0);
return null;
...
JW.Logger.error ("ERROR -- return from propertyExpression was " + vv);
}return tv;
}, "~N,~N,~S,~B");
$_M(c$, "setBitsetProperty", 
($fz = function (bs, tok, iValue, fValue, tokenValue) {
if (this.chk || JW.BSUtil.cardinalityOf (bs) == 0) return;
var list = null;
var sValue = null;
var fvalues = null;
var pt;
...
for (var i = nValues; --i >= 0; ) fvalues[i] = (tok == 1087375365 ? JW.Elements.elementNumberFromSymbol (list[i], false) : JU.PT.parseFloat (list[i]));
...
fixed[j] = J.script.T.tv (3, J.script.ScriptParam.getFloatEncodedInt ("" + v), v);
...
} else if (Clazz.instanceOf (v, J.script.ScriptContext)) {
fixed[j] = J.script.SV.newV (6, (v).getFullMap ());
