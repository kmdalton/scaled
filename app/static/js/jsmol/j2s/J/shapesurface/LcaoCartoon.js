...
Clazz.load (["J.shapesurface.Isosurface"], "J.shapesurface.LcaoCartoon", ["java.lang.Float", "JU.PT", "$.SB", "$.V3", "JW.C", "$.Escape", "$.Txt"], function () {
...
$_M(c$, "setLcaoOn", 
($fz = function (TF) {
if (JW.Txt.isWild (this.lcaoID)) {
var key = this.lcaoID.toLowerCase ();
for (var i = this.meshCount; --i >= 0; ) {
if (JW.Txt.isMatch (this.meshes[i].thisID.toLowerCase (), key, true, true)) this.meshes[i].visible = TF;
}
return;
...
if (JW.C.isColixTranslucent (colix)) {
this.setPropI ("translucentLevel", Float.$valueOf (JW.C.getColixTranslucencyLevel (colix)), null);
...
if (this.lcaoColorNeg != null) J.shape.Shape.appendCmd (sb, "lcaoCartoon color " + JW.Escape.escapeColor (this.lcaoColorNeg.intValue ()) + " " + JW.Escape.escapeColor (this.lcaoColorPos.intValue ()));
