...
Clazz.load (["JW.JmolMolecule", "JU.BS", "$.List", "J.smiles.VTemp"], "J.smiles.SmilesSearch", ["java.util.Arrays", "$.Hashtable", "JU.AU", "$.SB", "$.V3", "J.smiles.SmilesAromatic", "$.SmilesAtom", "$.SmilesBond", "$.SmilesMeasure", "$.SmilesParser", "JW.BSUtil", "$.Logger"], function () {
...
}, J.smiles, "SmilesSearch", JW.JmolMolecule);
...
if (JW.Logger.debugging && !this.isSilent) JW.Logger.debug ("SmilesSearch processing " + this.pattern);
...
if (JW.Logger.debugging && !this.isSilent) JW.Logger.debug ("pattern atom " + atomNum + " " + patternAtom);
...
var bs = JW.BSUtil.copy (this.bsFound);
...
}, $fz.isPrivate = true, $fz), "JW.Node,JU.BS");
...
return true;
}, $fz.isPrivate = true, $fz), "J.smiles.SmilesAtom,J.smiles.SmilesAtom,J.smiles.SmilesBond,~N,~N,JW.Edge");
...
}, $fz.isPrivate = true, $fz), "J.smiles.SmilesBond,~N,~N,JW.Edge");
...
if (JW.Logger.debugging) JW.Logger.debug ("checking stereochemistry...");
var atom1 = null;
var atom2 = null;
var atom3 = null;
var atom4 = null;
...
atom4 = null;
if (JW.Logger.debugging) JW.Logger.debug ("...type " + chiralClass + " for pattern atom " + sAtom + " " + atom0);
switch (chiralClass) {
case 2:
var isAllene = true;
if (isAllene) {
...
}, "~B,JW.Node,~N,~N,JW.Node,JW.Node,JW.Node,JW.Node,JW.Node,JW.Node,J.smiles.VTemp");
$_M(c$, "getJmolAtom", 
($fz = function (i) {
...
return true;
}, $fz.isPrivate = true, $fz), "JW.Node,J.smiles.SmilesAtom,J.smiles.SmilesAtom,~A");
$_M(c$, "getMappedAtoms", 
...
}, "JW.Node,JW.Node,JW.Node,JW.Node,J.smiles.VTemp,~N");
...
}, $fz.isPrivate = true, $fz), "JW.Node,JW.Node,JW.Node,JW.Node,J.smiles.VTemp");
