...
Clazz.load (["J.api.JmolZipUtility"], "J.io2.ZipUtil", ["java.io.BufferedInputStream", "$.BufferedReader", "java.lang.Character", "java.util.Hashtable", "$.StringTokenizer", "java.util.zip.CRC32", "$.GZIPInputStream", "$.ZipEntry", "$.ZipInputStream", "javajs.api.ZInputStream", "JU.List", "$.PT", "$.SB", "J.adapter.smarter.AtomSetCollection", "J.api.Interface", "J.io.JmolBinary", "J.io2.JmolZipInputStream", "JW.Escape", "$.Logger", "$.Txt"], function () {
...
var prefix = JW.Txt.join (subfileList, '/', 1);
var prefixd = null;
if (prefix != null) {
...
if (n == 0) return null;
JW.Logger.info ("ZipUtil cached " + n + " bytes from " + fileName);
...
var jd = J.api.Interface.getUtil ("CompoundDocument");
...
JW.Logger.info ("FileManager checking PNGJ cache for " + pathName);
