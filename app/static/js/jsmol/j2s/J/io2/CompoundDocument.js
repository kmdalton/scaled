...
Clazz.load (["J.io2.BinaryDocument", "JU.List", "J.io2.CompoundDocHeader"], "J.io2.CompoundDocument", ["java.io.DataInputStream", "JU.SB", "J.io2.CompoundDocDirEntry", "$.ZipData", "JW.Logger"], function () {
...
JW.Logger.info ("CompoundDocument file " + name);
...
for (var i = 0; i < this.directory.size (); i++) {
var thisEntry = this.directory.get (i);
JW.Logger.info ("reading " + thisEntry.entryName);
if (!thisEntry.isEmpty && thisEntry.entryType != 5) {
var name = thisEntry.entryName;
...
JW.Logger.debug ("compound document: revNum=" + this.header.revNumber + " verNum=" + this.header.verNumber + " isBigEndian=" + this.isBigEndian + " bytes per standard/short sector=" + this.sectorSize + "/" + this.shortSectorSize);
}return true;
...
if (JW.Logger.debugging) JW.Logger.debug ("CompoundDocument directory entry: \n" + this.getDirectoryListing ("\n"));
}, $fz.isPrivate = true, $fz));
