...
Clazz.load (["J.shape.Shape", "JU.P3i"], "J.shape.Sticks", ["java.util.Hashtable", "JU.BS", "$.P3", "J.constant.EnumPalette", "JW.BSUtil", "$.C", "$.Edge", "$.Escape"], function () {
...
if (size == 2147483647) {
this.selectedBonds = JW.BSUtil.copy (bsSelected);
return;
}if (size == -2147483648) {
...
bond.setColix (JW.C.getColix (JW.Edge.getArgbHbondType (bond.order)));
}}
return;
...
if (property.equals ("selectionState")) return (this.selectedBonds != null ? "select BONDS " + JW.Escape.eBS (this.selectedBonds) + "\n" : "");
