...
Clazz.load (["J.api.JmolMeasurementClient", "J.shape.AtomShape", "JU.List"], "J.shape.Measures", ["java.lang.Float", "java.util.Hashtable", "JU.BS", "$.PT", "JM.Measurement", "$.MeasurementData", "JW.BSUtil", "$.C", "$.Escape", "$.Txt"], function () {
...
this.setColor (value == null ? 0 : JW.C.getColixO (value));
return;
...
var bs = value;
if (bs == null || JW.BSUtil.cardinalityOf (bs) == 0) {
this.bsSelected = null;
} else {
...
var type = JM.Measurement.nmrType (this.viewer.getDistanceUnits (md.strFormat));
switch (type) {
case 2:
...
this.showHideM ( new JM.Measurement ().setPoints (this.modelSet, value, null, null), true);
}return;
}if ("show" === propertyName) {
if (Clazz.instanceOf (value, String)) {
this.doAction (null, value, 4148);
} else {
this.showHideM ( new JM.Measurement ().setPoints (this.modelSet, value, null, null), false);
}return;
}if ("toggle" === propertyName) {
if (Clazz.instanceOf (value, String)) {
this.doAction (null, value, 269484114);
} else {
this.toggle ( new JM.Measurement ().setPoints (this.modelSet, value, null, null));
}return;
...
return  new JM.Measurement ().setPoints (this.modelSet, indices, points, this.tickInfo == null ? this.defaultTickInfo : this.tickInfo);
...
}, $fz.isPrivate = true, $fz), "JM.Measurement,~B");
$_M(c$, "toggle", 
($fz = function (m) {
this.radiusData = null;
this.htMin = null;
...
this.setIndices ();
}, $fz.isPrivate = true, $fz), "JM.Measurement");
$_M(c$, "toggleOn", 
($fz = function (indices) {
this.radiusData = null;
this.htMin = null;
this.bsSelected =  new JU.BS ();
this.defineAll (-2147483648,  new JM.Measurement ().setPoints (this.modelSet, indices, null, this.defaultTickInfo), false, true, true);
this.setIndices ();
this.reformatDistances ();
...
this.defineAll (-2147483648,  new JM.Measurement ().setPoints (this.modelSet, value, null, null), true, false, false);
...
this.define (( new JM.MeasurementData ().init (null, this.viewer, points)).set (this.tokAction, this.htMin, this.radiusData, this.strFormat, null, this.tickInfo, this.mustBeConnected, this.mustNotBeConnected, this.intramolecular, true, 0, 0, null), (isDelete ? 12291 : 1060866));
}, $fz.isPrivate = true, $fz), "~N,JM.Measurement,~B,~B,~B");
$_M(c$, "find", 
($fz = function (m) {
...
}, $fz.isPrivate = true, $fz), "JM.MeasurementData,~N");
...
return;
}var measureNew =  new JM.Measurement ().setM (this.modelSet, m, value, (m.colix == 0 ? this.colix : m.colix), this.strFormat, this.measurementCount);
...
}, $fz.isPrivate = true, $fz), "~N,JM.Measurement,~B");
$_M(c$, "deleteI", 
($fz = function (i) {
...
var isWild = JW.Txt.isWild (s);
for (var i = this.measurements.size (); --i >= 0; ) {
var m = this.measurements.get (i);
if (m.thisID != null && (m.thisID.equalsIgnoreCase (s) || isWild && JW.Txt.isMatch (m.thisID.toUpperCase (), s, true, true))) switch (tok) {
case 1666189314:
m.mad = md.mad;
break;
case 12291:
...
}, $fz.isPrivate = true, $fz), "JM.MeasurementData,~S,~N");
...
atomInfo.put ("coord", JW.Escape.eP (m.getAtom (i)));
