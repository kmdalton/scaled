...
Clazz.load (["J.shape.AtomShape", "java.util.Hashtable"], "J.shape.Labels", ["javajs.awt.Font", "JU.AU", "$.BS", "$.List", "J.constant.EnumPalette", "JM.LabelToken", "$.Text", "J.script.SV", "JW.BSUtil", "$.C", "JV.JC"], function () {
...
var bgcolix = JW.C.getColixO (value);
...
tokensUNK = JM.LabelToken.compile (this.viewer, strLabelUNK, '\0', null);
}strLabel = strLabelUNK;
tokens = tokensUNK;
} else {
if (strLabelPDB == null) {
strLabelPDB = this.viewer.getStandardLabelFormat (2);
tokensPDB = JM.LabelToken.compile (this.viewer, strLabelPDB, '\0', null);
}strLabel = strLabelPDB;
tokens = tokensPDB;
}this.strings[i] = JM.LabelToken.formatLabelAtomArray (this.viewer, atom, tokens, '\0', null);
...
JW.BSUtil.deleteBits (this.bsFontSet, bsSelected);
JW.BSUtil.deleteBits (this.bsBgColixSet, bsSelected);
...
this.putLabel (i, t);
}, $fz.isPrivate = true, $fz), "~N,JM.Text");
$_M(c$, "setLabel", 
($fz = function (temp, strLabel, i) {
var atom = this.atoms[i];
var tokens = temp[0];
if (tokens == null) tokens = temp[0] = JM.LabelToken.compile (this.viewer, strLabel, '\0', null);
var label = (tokens == null ? null : JM.LabelToken.formatLabelAtomArray (this.viewer, atom, tokens, '\0', null));
...
}, $fz.isPrivate = true, $fz), "JM.Atom,~N,~S,~S");
$_V(c$, "getProperty", 
...
colix = JW.C.getColixInherited (colix, atom.getColix ());
if (JW.C.isColixTranslucent (colix)) colix = JW.C.getColixTranslucent3 (colix, false, 0);
}return colix;
}, "~N,JM.Atom,~B");
