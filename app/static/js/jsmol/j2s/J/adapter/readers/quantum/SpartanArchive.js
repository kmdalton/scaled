...
Clazz.load (null, "J.adapter.readers.quantum.SpartanArchive", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.AU", "$.List", "$.V3", "J.adapter.readers.quantum.SpartanSmolReader", "J.adapter.smarter.AtomSetCollectionReader", "$.Bond", "J.api.JmolAdapter", "J.constant.EnumQuantumShell", "JW.Logger"], function () {
...
JW.Logger.debug ("reading Spartan archive info :" + info);
...
if (doAddAtoms && JW.Logger.debugging) {
JW.Logger.debug (this.atomCount + " atoms read");
...
if (JW.Logger.debugging) {
JW.Logger.debug (bondCount + " bonds read");
}}, "~S,~N");
$_M(c$, "readBasis", 
function () {
...
if (JW.Logger.debugging) {
JW.Logger.debug (shells.size () + " slater shells read");
JW.Logger.debug (gaussians.length + " gaussian primitives read");
}});
...
if (JW.Logger.debugging) {
JW.Logger.debug (this.r.orbitals.size () + " molecular orbitals read");
}this.r.moData.put ("mos", this.r.orbitals);
});
$_M(c$, "readProperties", 
function () {
if (JW.Logger.debugging) JW.Logger.debug ("Reading PROPARC properties records...");
...
if (JW.Logger.debugging) {
JW.Logger.debug (" Skipping property line " + this.line);
...
if (JW.Logger.debugging) {
JW.Logger.debug ("reading VIBFREQ vibration records: frequencyCount = " + frequencyCount);
...
var freqs =  new JU.List ();
if (JW.Logger.debugging) {
JW.Logger.debug ("reading PROP VALUE:VIB FREQ_MODE vibration records: frequencyCount = " + frequencyCount);
}var v;
for (var i = 0; i < frequencyCount; ++i) {
var atomCount0 = this.r.atomSetCollection.atomCount;
this.r.atomSetCollection.cloneLastAtomSet ();
