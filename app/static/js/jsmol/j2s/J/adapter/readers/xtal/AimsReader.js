...
Clazz.load (["J.adapter.smarter.AtomSetCollectionReader"], "J.adapter.readers.xtal.AimsReader", ["JW.Logger"], function () {
...
JW.Logger.warn ("ignoring translation symmetry for more or less than 3 dimensions(which is currently neither supported by FHI-aims");
}this.finalizeReaderASCR ();
});
$_M(c$, "readLatticeVector", 
($fz = function (tokens) {
if (tokens.length < 4) {
JW.Logger.warn ("cannot read line with FHI-aims lattice vector: " + this.line);
} else if (this.nLatticeVectors == 3) {
JW.Logger.warn ("more than 3 FHI-aims lattice vectors found with line: " + this.line);
} else {
...
if (tokens.length < 5) {
JW.Logger.warn ("cannot read line with FHI-aims line: " + this.line);
return;
...
($fz = function (tokens) {
if (tokens.length < 6) {
JW.Logger.warn ("cannot read line with FHI-aims atom data: " + this.line);
return;
}var order = this.parseIntStr (tokens[4]);
if (order > 0) {
JW.Logger.warn ("multipole line ignored since only monopoles are currently supported: " + this.line);
return;
