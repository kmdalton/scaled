...
Clazz.load (["J.adapter.smarter.AtomSetCollectionReader", "J.api.JmolLineReader", "java.util.Hashtable", "JU.List", "$.P3", "J.io.CifDataReader"], "J.adapter.readers.cif.CifReader", ["java.lang.Boolean", "$.Character", "$.Float", "JU.BS", "$.PT", "$.V3", "J.adapter.smarter.Atom", "J.api.Interface", "$.JmolAdapter", "J.io.JmolBinary", "JW.Logger"], function () {
...
JW.Logger.warn ("CIF ERROR ? should be an underscore: " + this.key);
this.tokenizer.getTokenPeeked ();
...
atom.set (0, 0, 0);
var s = atom.atomName = this.tokenizer.fullTrim (this.data);
...
if (JW.Logger.debugging) JW.Logger.debug (this.key);
}, $fz.isPrivate = true, $fz));
$_M(c$, "nextAtomSet", 
($fz = function () {
...
JW.Logger.debug (type + " = " + this.data);
}return this.data;
}, $fz.isPrivate = true, $fz), "~S");
...
if (this.lattvecs != null && this.lattvecs.size () > 0 && this.atomSetCollection.getSymmetry ().addLatticeVectors (this.lattvecs)) this.appendLoadNote ("Note! Symmetry operators added for lattice centering " + this.latticeType);
this.latticeType = null;
}}, $fz.isPrivate = true, $fz));
...
if (this.data == null) {
JW.Logger.warn ("CIF ERROR ? end of file; data missing: " + this.key);
return false;
...
JW.Logger.warn ("ignoring file-based symmetry operators");
this.skipLoop ();
} else {
...
if (Math.abs (atom.formalCharge - charge) > 0.1) if (JW.Logger.debugging) {
JW.Logger.debug ("CIF charge on " + this.field + " was " + charge + "; rounded to " + atom.formalCharge);
}}break;
case 49:
case 1:
case 2:
...
JW.Logger.info ("ignoring " + atom.atomName);
return false;
}}}return true;
...
if (nRefs != 1) {
JW.Logger.warn ("?que? _symmetry_equiv or _space_group_symop property not found");
this.skipLoop ();
return;
}var n = 0;
while (this.tokenizer.getData ()) {
...
default:
JW.Logger.warn ("unknown CIF bond order: " + field);
case 'S':
return 1;
case 'D':
return 2;
...
JW.Logger.warn ("?que? missing property: " + J.adapter.readers.cif.CifReader.geomBondFields[i]);
this.skipLoop ();
return;
}
var name1 = null;
...
if (Float.isNaN (dx)) {
JW.Logger.info ("error reading uncertainty for " + this.line);
dx = 0.015;
}} else {
dx = 0.015;
}break;
...
if (bondCount > 0) {
JW.Logger.info (bondCount + " bonds read");
if (!this.doApplySymmetry) {
...
($fz = function () {
JW.Logger.info ("CIF creating molecule " + (this.bondTypes.size () > 0 ? " using GEOM_BOND records" : ""));
...
if (this.atomSetCollection.bsAtoms.get (i)) this.symmetry.toCartesian (this.atoms[i], true);
 else if (JW.Logger.debugging) JW.Logger.debug (this.molecularType + " removing " + i + " " + this.atoms[i].atomName + " " + this.atoms[i]);
}
