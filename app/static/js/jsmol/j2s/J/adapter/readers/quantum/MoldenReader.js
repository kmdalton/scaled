...
Clazz.load (["J.adapter.readers.quantum.MopacSlaterReader", "JU.BS"], "J.adapter.readers.quantum.MoldenReader", ["java.lang.Exception", "$.Float", "java.util.Arrays", "$.Hashtable", "JU.AU", "$.List", "$.PT", "J.adapter.readers.quantum.BasisFunctionReader", "J.api.JmolAdapter", "JW.Logger"], function () {
...
JW.Logger.info ("MoldenReader assigning shells starting with " + i + " for ** to atom " + (j + 1) + " z " + ilast);
...
JW.Logger.error ("Molden reader could not assign shells -- abandoning MOs");
...
JW.Logger.info (this.shells.size () + " slater shells read");
JW.Logger.info (garray.length + " gaussian primitives read");
JW.Logger.info (this.nCoef + " MO coefficients expected for orbital type " + this.orbitalType);
...
JW.Logger.info ("too few orbital coefficients for 6D");
...
JW.Logger.debug (coefs.length + " coefficients in MO " + this.orbitals.size ());
}}this.line = l;
}
if (JW.Logger.debugging) JW.Logger.debug ("read " + this.orbitals.size () + " MOs");
this.setMOs ("eV");
...
JW.Logger.info ("Orbital type set to " + this.orbitalType);
this.fixOrbitalType ();
return true;
}return false;
}, $fz.isPrivate = true, $fz), "~S");
