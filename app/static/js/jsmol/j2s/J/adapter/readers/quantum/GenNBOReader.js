...
Clazz.load (["J.adapter.readers.quantum.MOReader"], "J.adapter.readers.quantum.GenNBOReader", ["java.lang.Boolean", "$.Character", "$.Exception", "$.Float", "java.util.Hashtable", "JU.AU", "$.List", "$.SB", "J.api.JmolAdapter", "J.io.JmolBinary", "JW.Logger"], function () {
...
}if (!isOK) JW.Logger.error ("Unimplemented shell type -- no orbitals avaliable: " + this.line);
...
JW.Logger.debug (this.shells.size () + " slater shells read");
JW.Logger.debug (this.gaussians.length + " gaussian primitives read");
}return true;
...
JW.Logger.error ("file 46 number of orbitals does not match nOrbitals: " + this.nOrbitals);
return false;
}var ntype = null;
...
if (ntype == null) {
JW.Logger.error ("uninterpretable type " + this.moType);
return false;
