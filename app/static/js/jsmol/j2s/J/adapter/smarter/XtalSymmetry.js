...
Clazz.load (["JU.P3"], "J.adapter.smarter.XtalSymmetry", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.BS", "$.M3", "$.M4", "$.P3i", "$.PT", "$.SB", "$.V3", "J.adapter.smarter.Atom", "J.api.Interface", "JW.BSUtil", "$.Escape", "$.Logger"], function () {
...
function (pt) {
this.ptSupercell = pt;
JW.Logger.info ("Using supercell " + JW.Escape.eP (pt));
}, "JU.P3");
$_M(c$, "setSuperCell", 
...
atom1.bsSymmetry = JW.BSUtil.newAndSetBit (iCellOpPt + iSym);
...
JW.Logger.error ("Cannot apply biomolecule when lattice cells are indicated");
return;
...
}for (var iAtom = this.firstSymmetryAtom; iAtom < atomMax; iAtom++) atoms[iAtom].bsSymmetry = JW.BSUtil.newAndSetBit (0);
...
atom1.atomSite = atomSite;
mat.rotTrans (atom1);
atom1.bsSymmetry = JW.BSUtil.newAndSetBit (i);
if (addBonds) {
for (var bondNum = this.ac.bondIndex0; bondNum < this.bondCount0; bondNum++) {
...
}, "J.adapter.smarter.Atom,JW.Tensor,~N,~B,J.api.SymmetryInterface");
$_M(c$, "setTensors", 
function () {
