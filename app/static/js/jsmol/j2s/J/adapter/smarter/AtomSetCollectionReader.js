...
Clazz.load (["JU.SB"], "J.adapter.smarter.AtomSetCollectionReader", ["java.io.BufferedReader", "java.lang.Boolean", "$.Character", "$.Float", "JU.BS", "$.List", "$.M3", "$.P3", "$.PT", "$.Quat", "$.V3", "J.adapter.smarter.Atom", "$.AtomSetCollection", "J.api.Interface", "$.JmolAdapter", "$.JmolDocument", "JW.BSUtil", "$.Logger", "$.Parser"], function () {
...
if (Clazz.instanceOf (reader, java.io.BufferedReader)) this.reader = reader;
 else if (Clazz.instanceOf (reader, J.api.JmolDocument)) this.binaryDoc = reader;
}, "~S,java.util.Map,~O");
$_M(c$, "readData", 
function () {
this.initialize ();
...
if (this.htParams.containsKey ("bsFilter")) this.htParams.put ("filteredAtomCount", Integer.$valueOf (JW.BSUtil.cardinalityOf (this.htParams.get ("bsFilter"))));
...
this.bsModels = JW.BSUtil.newAndSetBit (this.firstLastStep[0]);
...
JW.Logger.info ("Setting space group name to " + this.spaceGroup);
}, "~S");
...
if (isym < 0) JW.Logger.warn ("Skipping symmetry operation " + xyz);
...
JW.Logger.debug ("setunitcellitem " + i + " " + x);
...
JW.Logger.info ("filtering with " + this.filter);
...
this.atomSetCollection.setAtomSetCollectionAuxiliaryInfo ("defaultOrientationMatrix", JU.M3.newM3 (this.matrixRotate));
var q = JU.Quat.newM (this.matrixRotate);
this.atomSetCollection.setAtomSetCollectionAuxiliaryInfo ("defaultOrientationQuaternion", q);
JW.Logger.info ("defaultOrientationMatrix = " + this.matrixRotate);
}, "~N,~N,~N,~N,~N,~N,~N,~N,~N");
...
if (orbitals != null) JW.Logger.info (orbitals.size () + " molecular orbitals read in model " + this.atomSetCollection.atomSetCount);
}, "java.util.Map");
...
if (iAtom < 0) continue;
if (JW.Logger.debugging) JW.Logger.debug ("atom " + iAtom + " vib" + j + ": " + vx + " " + vy + " " + vz);
...
$_M(c$, "addJmolScript", 
function (script) {
JW.Logger.info ("#jmolScript: " + script);
...
function (s, data) {
JW.Parser.parseStringInfestedFloatArray (s, null, data);
}, "~S,~A");
