...
Clazz.load (["java.util.Hashtable"], "J.adapter.smarter.AtomSetCollection", ["java.lang.Boolean", "$.Float", "java.util.Collections", "$.Properties", "JU.AU", "$.BS", "$.List", "$.P3", "$.V3", "J.adapter.smarter.Atom", "$.Bond", "$.SmarterJmolAdapter", "J.api.Interface", "JW.Logger"], function () {
...
JW.Logger.debug (">>>>>>BAD BOND:" + bond.atomIndex1 + "-" + bond.atomIndex2 + " order=" + bond.order);
}return;
...
JW.Logger.info ("Setting partial charges type " + auxKey);
return true;
}, "~S");
...
if (this.currentAtomSetIndex < 0) return;
JW.Logger.info ("Energy for model " + (this.currentAtomSetIndex + 1) + " = " + energyString);
