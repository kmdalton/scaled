...
Clazz.load (["J.shapespecial.DrawMesh", "JU.List"], "J.shapecgo.CGOMesh", ["java.lang.Float", "$.Short", "JU.BS", "$.CU", "JW.C", "$.Logger", "$.Normix"], function () {
...
if (len < 0) {
JW.Logger.error ("CGO unknown type: " + type);
return false;
}switch (type) {
case 1:
len = 8;
...
JW.Logger.error ("CGOMesh error: " + e);
this.cmds = null;
return false;
} else {
throw e;
...
this.cList.addLast (Short.$valueOf (JW.C.getColix (JU.CU.colorPtToFFRGB (this.vTemp))));
}, $fz.isPrivate = true, $fz), "~N");
$_M(c$, "addNormix", 
($fz = function (i) {
this.getPoint (i, this.vTemp);
this.nList.addLast (Short.$valueOf (JW.Normix.get2SidedNormix (this.vTemp, this.bsTemp)));
}, $fz.isPrivate = true, $fz), "~N");
$_M(c$, "getPoint", 
function (i, pt) {
