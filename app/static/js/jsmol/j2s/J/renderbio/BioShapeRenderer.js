...
Clazz.load (["J.render.MeshRenderer", "JU.A4", "$.BS", "$.M3", "$.P3", "$.P3i", "$.V3"], "J.renderbio.BioShapeRenderer", ["J.constant.EnumStructure", "JMB.CarbohydratePolymer", "$.NucleicPolymer", "J.shape.Mesh", "JW.C", "$.Hermite", "$.Logger", "$.Normix"], function () {
...
this.isNucleic = Clazz.instanceOf (bioShape.bioPolymer, JMB.NucleicPolymer);
this.isCarbohydrate = Clazz.instanceOf (bioShape.bioPolymer, JMB.CarbohydratePolymer);
this.haveControlPointScreens = false;
...
$_M(c$, "getLeadColix", 
function (i) {
return JW.C.getColixInherited (this.colixes[i], this.monomers[i].getLeadAtom ().getColix ());
}, "~N");
...
this.meshes[i] = null;
JW.Logger.error ("render mesh error hermiteConic: " + e.toString ());
} else {
throw e;
}
}
...
this.bsRenderMesh.clear (i);
this.meshes[i] = null;
JW.Logger.error ("render mesh error hermiteRibbon: " + e.toString ());
} else {
throw e;
}
}
...
this.bsRenderMesh.clear (i);
this.meshes[i] = null;
JW.Logger.error ("render mesh error hermiteArrowHead: " + e.toString ());
} else {
throw e;
}
}
...
}JW.Hermite.getHermiteList (this.isNucleic ? 4 : 7, this.controlPoints[this.iPrev], this.controlPoints[i], this.controlPoints[this.iNext], this.controlPoints[this.iNext2], this.controlPoints[this.iNext3], this.controlHermites, 0, nHermites, true);
...
JW.Hermite.getHermiteList (this.isNucleic ? 4 : 7, this.wing, this.wingVectors[i], this.wingVectors[this.iNext], this.wingVectors[this.iNext2], this.wingVectors[this.iNext3], this.wingHermites, 0, nHermites, false);
...
JW.Hermite.getHermiteList (4, this.ptPrev, this.pt, this.pt1, this.ptNext, this.ptNext, this.radiusHermites, 0, (nHermites + 1) >> 1, true);
}var nPoints = 0;
