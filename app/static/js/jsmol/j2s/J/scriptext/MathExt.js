...
Clazz.load (["J.script.JmolMathExtension"], "J.scriptext.MathExt", ["java.lang.Float", "java.util.Date", "JU.AU", "$.BS", "$.CU", "$.List", "$.M3", "$.M4", "$.P3", "$.P4", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.constant.EnumVdw", "J.i18n.GT", "JM.BondSet", "J.script.SV", "$.ScriptParam", "$.T", "JW.BSUtil", "$.Escape", "$.JmolMolecule", "$.Logger", "$.Measure", "$.Parser", "$.Point3fi", "$.Txt"], function () {
...
var mf1 = (bs1 == null ? this.viewer.getSmilesMatcher ().getMolecularFormula (smiles1, false) : JW.JmolMolecule.getMolecularFormula (this.viewer.getModelSet ().atoms, bs1, false));
var mf2 = (bs2 == null ? this.viewer.getSmilesMatcher ().getMolecularFormula (smiles2, false) : JW.JmolMolecule.getMolecularFormula (this.viewer.getModelSet ().atoms, bs2, false));
...
return mp.addX (J.script.SV.newV (10,  new JM.BondSet (bsBonds, this.viewer.getAtomIndices (this.viewer.getAtomBits (1678770178, bsBonds)))));
}return mp.addXBs (this.viewer.modelSet.getAtomsConnected (min, max, order, atoms1));
}, $fz.isPrivate = true, $fz), "J.script.ScriptMathProcessor,~A");
...
var bs = (Clazz.instanceOf (args[0].value, JU.BS) ? args[0].value : this.viewer.getAtomBits (1095761939,  new Integer (args[0].asInt ())));
switch (tok) {
case 135266320:
...
try {
if (isChemical) {
var data = (x1.tok == 10 ? this.viewer.getSmiles (J.script.SV.getBitSet (x1, false)) : J.script.SV.sValue (x1));
data = data.length == 0 ? "" : this.viewer.getChemicalInfo (data, args.length > 1 ? J.script.T.getTokenFromName (flags.toLowerCase ()) : null);
...
case 10:
if (isMF) return mp.addXStr (JW.JmolMolecule.getMolecularFormula (this.viewer.getModelSet ().atoms, x1.value, false));
if (isSequence) return mp.addXStr (this.viewer.getSmilesOpt (x1.value, -1, -1, false, true, isAll, isAll, false));
...
return mp.addXObj ((this.viewer.newMeasurementData (null, points)).set (0, null, rd, strFormat, units, null, isAllConnected, isNotConnected, null, true, 0, 0, null).getMeasurements (asArray, asMinArray));
case 135266305:
...
switch (nArgs) {
case 0:
return mp.addXPt4 (JU.Quat.newQ (this.viewer.getRotationQuaternion ()).toPoint4f ());
case 1:
default:
...
var s = J.script.SV.sValue (args[0]);
var v = JW.Escape.uP (s.equalsIgnoreCase ("best") ? this.viewer.getOrientationText (1073741863, null) : s);
...
if (this.e.getShapePropertyData (24, "getVertices", data)) return this.viewer.getAtomsNearPts (distance, data[1], data[2]);
data[1] = Integer.$valueOf (0);
data[2] = Integer.$valueOf (-1);
if (this.e.getShapePropertyData (22, "getCenter", data)) return this.viewer.getAtomsNearPt (distance, data[2]);
return  new JU.BS ();
...
$_V(c$, "setContactBitSets", 
function (bsA, bsB, localOnly, distance, rd, warnMultiModel) {
var withinAllModels;
var bs;
if (bsB == null) {
bsB = JW.BSUtil.setAll (this.viewer.getAtomCount ());
JW.BSUtil.andNot (bsB, this.viewer.getDeletedAtoms ());
bsB.andNot (bsA);
