...
Clazz.load (["J.script.JmolSmilesExtension"], "J.scriptext.SmilesExt", ["java.lang.Float", "JU.AU", "$.BS", "$.List", "$.M4", "$.P3", "JW.Escape", "$.Logger", "$.Measure"], function () {
c$ = Clazz.decorateAsClass (function () {
this.e = null;
this.sm = null;
Clazz.instantialize (this, arguments);
}, J.scriptext, "SmilesExt", null, J.script.JmolSmilesExtension);
...
function (se) {
this.e = se;
this.sm = this.e.viewer.getSmilesMatcher ();
return this;
}, "~O");
$_V(c$, "getSmilesCorrelation", 
...
var c =  new JU.P3 ();
var atoms = this.e.viewer.modelSet.atoms;
var atomCount = this.e.viewer.getAtomCount ();
var maps = this.sm.getCorrelationMaps (smiles, atoms, atomCount, bsA, isSmarts, true);
...
maps = this.sm.getCorrelationMaps (smiles, atoms, atomCount, bsB, isSmarts, firstMatchOnly);
if (maps == null) this.e.evalError (this.sm.getLastException (), null);
if (maps.length == 0) return NaN;
JW.Logger.info (maps.length + " mappings found");
if (bestMap || !asMap) {
...
var stddev = JW.Measure.getTransformMatrix4 (ptsA, ptsB, m, c, false);
JW.Logger.info ("getSmilesCorrelation stddev=" + stddev);
if (vReturn != null) {
if (stddev < tolerance) {
var bs =  new JU.BS ();
...
try {
return this.e.viewer.getSmilesOpt (bsSelected, 0, 0, pattern.equals ("H"), isBioSmiles, false, true, true);
} catch (ex) {
if (Clazz.exceptionOf (ex, Exception)) {
this.e.evalError (ex.getMessage (), null);
} else {
...
asAtoms = (smiles == null);
try {
if (asAtoms) b = this.sm.getSubstructureSetArray (pattern, this.e.viewer.modelSet.atoms, this.e.viewer.getAtomCount (), bsSelected, null, isSmarts, false);
...
for (var j = 0; j < b.length; j++) matches[j] = (asAtoms ? JW.Escape.eBS (b[j]) : JW.Escape.eBond (b[j]));
...
var bondMap1 = this.e.viewer.getDihedralMap (mapSet[0]);
var bondMap2 = (bondMap1 == null ? null : this.e.viewer.getDihedralMap (mapSet[1]));
if (bondMap2 == null || bondMap2.length != bondMap1.length) return null;
var angles =  Clazz.newFloatArray (bondMap1.length, 3, 0);
var atoms = this.e.viewer.modelSet.atoms;
J.scriptext.SmilesExt.getTorsions (atoms, bondMap2, angles, 0);
J.scriptext.SmilesExt.getTorsions (atoms, bondMap1, angles, 1);
...
var map = bondMap[i];
var v = JW.Measure.computeTorsion (atoms[map[0]], atoms[map[1]], atoms[map[2]], atoms[map[3]], true);
if (pt == 1) {
