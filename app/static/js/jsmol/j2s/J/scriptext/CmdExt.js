...
Clazz.load (["J.script.JmolCmdExtension"], "J.scriptext.CmdExt", ["java.lang.Boolean", "$.Float", "$.Long", "$.Short", "java.util.Hashtable", "JU.AU", "$.BS", "$.List", "$.M3", "$.M4", "$.P3", "$.PT", "$.Quat", "$.SB", "$.V3", "J.api.Interface", "J.atomdata.RadiusData", "J.constant.EnumAxesMode", "$.EnumStereoMode", "$.EnumVdw", "J.i18n.GT", "JM.Atom", "$.AtomCollection", "$.BondSet", "$.LabelToken", "J.script.SV", "$.ScriptCompiler", "$.ScriptError", "$.ScriptEval", "$.ScriptInterruption", "$.ScriptMathProcessor", "$.ScriptParam", "$.T", "JW.BSUtil", "$.BoxInfo", "$.C", "$.Edge", "$.Elements", "$.Escape", "$.Logger", "$.Measure", "$.Parser", "$.Point3fi", "$.TempArray", "$.Txt", "JV.FileManager", "$.JC", "$.StateManager", "$.Viewer"], function () {
...
}, J.scriptext, "CmdExt", null, J.script.JmolCmdExtension);
...
if (asDSSP) bs1 = this.viewer.getSelectedAtoms ();
...
bs1 = (this.e.iToken + 1 < this.slen ? this.atomExpressionAt (++this.e.iToken) : this.viewer.getSelectedAtoms ());
this.checkLength (++this.e.iToken);
...
bsAtoms2 = (coordTo == null && this.e.isArrayParameter (this.e.iToken + 1) ? null : (tok == 10 || tok == 1048577 ? this.atomExpressionAt (++this.e.iToken) : JW.BSUtil.copy (bsAtoms1)));
if (bsSubset != null) {
...
bsAtoms2 = JW.BSUtil.copy (bsAtoms1);
bsAtoms1.and (bsFrom);
bsAtoms2.and (bsTo);
}} else {
bsAtoms1 = JW.BSUtil.copy (bsFrom);
bsAtoms2 = JW.BSUtil.copy (bsTo);
bsAtoms1.and (bsSubset);
...
if (isFrames) vAtomSets2.addLast (bss = [JW.BSUtil.copy (bss[0]), bss[1]]);
bss[0].and (bsFrom);
}
var center = null;
...
q = JW.Measure.calculateQuaternionRotation (centerAndPoints, retStddev, true);
...
ptsB = JW.Measure.transformPoints (ptsA, m4, center);
}if (!this.e.useThreads ()) doAnimate = false;
if (this.viewer.rotateAboutPointsInternal (this.e, center, pt1, endDegrees / nSeconds, endDegrees, doAnimate, bsFrom, translation, ptsB, null) && doAnimate && this.e.isJS) throw  new J.script.ScriptInterruption (this.e, "compare", 1);
}
}, $fz.isPrivate = true, $fz));
$_M(c$, "configuration", 
($fz = function () {
...
this.viewer.addStateScriptRet ("select", null, this.viewer.getSelectedAtoms (), null, "configuration", true, false);
} else {
...
this.viewer.select (bsAtoms, false, 0, this.e.tQuiet);
}, $fz.isPrivate = true, $fz));
$_M(c$, "measure", 
($fz = function () {
var eval = this.e;
...
break;
case 1766856708:
colix = JW.C.getColix (eval.getArgbParam (++i));
i = eval.iToken;
break;
case 1073742066:
if (eval.isPoint3f (++i)) {
...
nBitSets++;
nAtoms++;
var bs2 = JW.BSUtil.copy (bs);
JW.BSUtil.invertInPlace (bs2, this.viewer.getAtomCount ());
...
var text = null;
if (font != null) text = (J.api.Interface.getInterface ("JM.Text")).newLabel (this.viewer.getGraphicsData (), font, "", colix, 0, 0, 0, null);
if (text != null) text.pymolOffset = offset;
this.setShapeProperty (6, "measure", this.viewer.newMeasurementData (id, points).set (tokAction, null, rd, strFormat, null, tickInfo, isAllConnected, isNotConnected, intramolecular, isAll, mad, colix, text));
return;
...
var addGroup = false;
if (this.slen == 1) {
if (!this.chk) this.viewer.rebondState (eval.$isStateScript);
return;
...
bo = JW.Edge.getBondOrderFromFloat (this.floatParameter (i));
...
if (!this.chk) this.viewer.setPdbConectBonding (isAuto, eval.$isStateScript);
return;
case 1073741830:
case 1073741852:
case 1073741904:
...
if ((bo = J.script.ScriptParam.getBondOrderFromString (cmd)) == 131071) {
this.invArg ();
...
case 3:
bo = J.script.ScriptParam.getPartialBondOrderFromFloatEncodedInt (this.st[++i].intValue);
break;
case 2:
...
bs = JW.BSUtil.copy (this.atomExpressionAt (i));
i = eval.iToken;
if (bsA == null) bsA = bs;
 else bsB = bs;
sbCommand.append (" ").append (JW.Escape.eBS (bs));
break;
}
idSeen = (eval.theTok != 12291);
}
...
}if (params != null) sbCommand.append (" parameters ").append (JW.Escape.eAF (params));
...
if (!this.chk) this.e.runScript (this.viewer.getPointGroupAsString (true, type, index, scale));
return false;
case 137363467:
case 135270418:
case 1052714:
...
if (isFrame) {
eval.checkLast (eval.iToken);
if (!this.chk) eval.runScript (JW.Escape.drawQuat (JU.Quat.newP4 (propertyValue), (thisId == null ? "frame" : thisId), " " + swidth, (center == null ?  new JU.P3 () : center), intScale / 100));
return false;
...
this.viewer.connect (this.parseDataArray (dataString, false));
return;
...
var atomCount = this.viewer.getAtomCount ();
var atomMap = null;
var bsTemp = JU.BS.newN (atomCount);
...
d[2] = JW.BSUtil.copy (bs);
}d[1] = dataString;
d[3] = Integer.$valueOf (0);
this.viewer.setData (dataType, d, atomCount, atomNumberField, atomNumberFieldColumnCount, propertyField, propertyFieldColumnCount);
return;
...
d[1] = dataString;
d[3] = Integer.$valueOf (0);
this.viewer.setData (dataType, d, 0, 0, 0, 0, 0);
});
$_M(c$, "ellipsoid", 
($fz = function () {
var eval = this.e;
...
if (bsSelect == null) bsSelect = JW.BSUtil.copy (this.viewer.getSelectedAtoms ());
...
if (needIgnore) {
bsIgnore = JW.BSUtil.copy (bsSelect);
JW.BSUtil.invertInPlace (bsIgnore, this.viewer.getAtomCount ());
isFrontOnly = true;
this.addShapeProperty (propertyList, "ignore", bsIgnore);
sbCommand.append (" ignore ").append (JW.Escape.eBS (bsIgnore));
...
}sbCommand.append (" boundBox " + JW.Escape.eP (pts[0]) + " " + JW.Escape.eP (pts[pts.length - 1]));
...
sbCommand.append (" intersection ").append (JW.Escape.eBS (bsSelect)).append (" ").append (JW.Escape.eBS (bs));
i = eval.iToken;
...
sbCommand.append (" ").append (JW.Escape.eBS (propertyValue));
}eval.checkLast (i);
break;
...
sbCommand.append (" within ").appendF (distance).append (" ").append (bs == null ? JW.Escape.eP (ptc) : JW.Escape.eBS (bs));
}continue;
case 1073742083:
...
sbCommand.append (" parameters ").append (JW.Escape.eAF (fparams));
break;
case 1716520985:
case 1073742190:
...
data =  Clazz.newFloatArray (atomCount, 0);
if (!this.chk) JW.Parser.parseStringInfestedFloatArray ("" + eval.getParameter (vname, 4), null, data);
}if (!this.chk) sbCommand.append (" \"\" ").append (JW.Escape.eAF (data));
} else {
this.getToken (++i);
if (!this.chk) {
...
if (tokProperty != 1766856708) for (var iAtom = atomCount; --iAtom >= 0; ) data[iAtom] = JM.Atom.atomPropertyFloat (this.viewer, atoms[iAtom], tokProperty);
...
if (isOnly) {
i++;
bsIgnore = JW.BSUtil.copy (bs1);
JW.BSUtil.invertInPlace (bsIgnore, this.viewer.getAtomCount ());
this.addShapeProperty (propertyList, "ignore", bsIgnore);
sbCommand.append (" ignore ").append (JW.Escape.eBS (bsIgnore));
isFrontOnly = true;
}if (surfaceObjectSeen || isMapped) {
sbCommand.append (" select " + JW.Escape.eBS (bs1));
} else {
...
propertyName = "center";
propertyValue = this.centerParameter (++i);
sbCommand.append (" center " + JW.Escape.eP (propertyValue));
i = eval.iToken;
break;
case 1073742147:
case 1766856708:
...
discreteColixes = JW.C.getColixArray (colorScheme);
...
sbCommand.append (" ").append (JW.Escape.escapeColor (color));
i = eval.iToken;
continue;
...
i = eval.iToken;
sbCommand.append (" plane ").append (JW.Escape.eP4 (propertyValue));
break;
case 1073742138:
propertyName = "scale";
propertyValue = Float.$valueOf (this.floatParameter (++i));
...
i = eval.iToken;
sbCommand.append (" ellipsoid ").append (JW.Escape.eP4 (propertyValue));
break;
case 135267841:
planeSeen = true;
propertyName = "plane";
...
sbCommand.append (" anisotropy").append (JW.Escape.eP (propertyValue));
i = eval.iToken;
break;
case 1073741842:
doCalcArea = !this.chk;
...
sbCommand.append (" discrete ").append (JW.Escape.eAF (propertyValue));
i = eval.iToken;
break;
case 1073741981:
...
propertyValue = pt;
i = eval.iToken;
sbCommand.append (" increment ").append (JW.Escape.eP (pt));
break;
default:
...
sbCommand.append (JW.Escape.eAF (propertyValue));
i = eval.iToken;
}break;
case 1073741928:
...
propertyValue = this.getPoint4f (++i);
sbCommand.append (" eccentricity ").append (JW.Escape.eP4 (propertyValue));
i = eval.iToken;
break;
case 1074790508:
sbCommand.append (" ed");
...
sbCommand.append (" ").append (JW.Escape.eP (pt3));
vxy.addLast (pt3);
var pt4;
ptX = ++eval.iToken;
vxy.addLast (pt4 = this.getPoint4f (ptX));
sbCommand.append (" ").append (JW.Escape.eP4 (pt4));
...
sbCommand.append (" ").append (JW.Escape.e (xyzdata));
} else {
if (isInline) {
nX = Math.abs (nX);
nY = Math.abs (nY);
...
vxy.addLast (fdata);
sbCommand.append (" ").append (JW.Escape.e (fdata));
}}i = eval.iToken;
break;
case 1073741970:
...
propertyValue = this.getPoint4f (++i);
i = eval.iToken;
sbCommand.append (" lobe ").append (JW.Escape.eP4 (propertyValue));
surfaceObjectSeen = true;
break;
case 1073742004:
case 1073742006:
...
propertyValue = this.getPoint4f (++i);
i = eval.iToken;
sbCommand.append (" lp ").append (JW.Escape.eP4 (propertyValue));
surfaceObjectSeen = true;
break;
case 1052700:
...
sbCommand.append ("plane ").append (JW.Escape.eP4 (plane));
planeSeen = true;
plane = null;
...
propertyValue = this.getPoint4f (++i);
i = eval.iToken;
sbCommand.append (" radical ").append (JW.Escape.eP4 (propertyValue));
break;
case 1073742027:
propertyName = "fixed";
propertyValue = Boolean.FALSE;
...
if (eval.debugHigh) JW.Logger.debug ("pmesh inline data:\n" + str);
...
sbCommand.append (" within 2.0 ").append (JW.Escape.eBS (bs));
...
}JW.Logger.info ("reading isosurface data from " + filename);
if (stype != null) {
...
sbCommand.append (" lattice ").append (JW.Escape.eP (pt));
if (isMapped) {
...
if (needSelect) bsSelect = JW.BSUtil.copy (this.viewer.getSelectedAtoms ());
if (modelIndex < 0) modelIndex = this.viewer.getCurrentModelIndex ();
...
sbCommand =  new JU.SB ().append ("; isosurface map select ").append (JW.Escape.eBS (bsSelect)).append (sbCommand.substring (16));
...
if (timeMsg) JW.Logger.startTimer ("isosurface");
this.setShapeProperty (iShape, "setProperties", propertyList);
if (timeMsg) this.showString (JW.Logger.getTimerMsg ("isosurface", 0));
if (defaultMesh) {
...
this.setShapeProperty (iShape, "finalize", (cmd.indexOf ("; isosurface map") == 0 ? "" : " select " + JW.Escape.eBS (bsSelect) + " ") + cmd);
...
JW.Logger.error ("sigma not supported");
}s += " created";
...
sarea = (doCalcArea ? "isosurfaceArea = " + (Clazz.instanceOf (area, Float) ? "" + area : JW.Escape.eAD (area)) : null);
svol = (doCalcVolume ? "isosurfaceVolume = " + (Clazz.instanceOf (volume, Float) ? "" + volume : JW.Escape.eAD (volume)) : null);
if (s == null) {
...
bsFrom = this.viewer.getSelectedAtoms ();
bsTo = this.atomExpressionAt (2);
property1 = property2 = "selected";
} else {
bsFrom = this.atomExpressionAt (1);
if (this.tokAt (++this.e.iToken) != 1048583 || !J.script.T.tokAttr (tokProp1 = this.tokAt (++this.e.iToken), 1078984704)) break;
property1 = this.paramAsStr (this.e.iToken);
bsTo = this.atomExpressionAt (++this.e.iToken);
if (this.tokAt (++this.e.iToken) != 1048583 || !J.script.T.tokAttr (tokProp2 = this.tokAt (++this.e.iToken), 2048)) break;
property2 = this.paramAsStr (this.e.iToken);
}if (J.script.T.tokAttr (tokKey = this.tokAt (this.e.iToken + 1), 1078984704)) mapKey = this.paramAsStr (++this.e.iToken);
...
if (JW.Logger.debugging) JW.Logger.debug (sb.toString ());
var bsSubset = JW.BSUtil.copy (this.viewer.getSelectionSubset ());
...
case 135270422:
makeNewFrame = false;
if (J.scriptext.CmdExt.tokAtArray (pt, args) == 4) {
filename = this.stringParameter (pt--);
} else if (J.scriptext.CmdExt.tokAtArray (pt - 1, args) == 1048583) {
...
var propertyY = 0;
var propertyZ = 0;
var bs = JW.BSUtil.copy (this.viewer.getSelectedAtoms ());
var preSelected = "; select " + JW.Escape.eBS (bs) + ";\n ";
var type = this.e.optParameterAsString (pt).toLowerCase ();
var minXYZ = null;
var maxXYZ = null;
var tok = J.scriptext.CmdExt.tokAtArray (pt0, args);
...
case 1716520985:
this.e.iToken = pt0 + 1;
if (!J.script.T.tokAttr (propertyX = this.tokAt (this.e.iToken++), 1078984704) || !J.script.T.tokAttr (propertyY = this.tokAt (this.e.iToken++), 1078984704)) this.invArg ();
if (J.script.T.tokAttr (propertyZ = this.tokAt (this.e.iToken), 1078984704)) this.e.iToken++;
 else propertyZ = 0;
...
stateScript = "select " + JW.Escape.eBS (bs) + ";\n ";
break;
case 1052714:
...
JW.Logger.info ("plot min/max: " + minXYZ + " " + maxXYZ);
...
if (JW.Logger.debugging) JW.Logger.debug (data);
if (tokCmd == 135176) {
...
var color = (JW.C.getHexCode (this.viewer.getColixBackgroundContrast ()));
...
var se =  new J.script.ScriptEval ().setViewer (this.viewer);
se.historyDisabled = true;
se.compiler =  new J.script.ScriptCompiler (this.viewer);
se.sm = shapeManager;
try {
...
if (Clazz.exceptionOf (ex, Exception)) {
this.e.viewer.setStringProperty ("_errormessage", "" + ex);
if (se.thisContext == null) {
JW.Logger.error ("Error evaluating context " + ex);
...
pt = pt0 = 1;
isCommand = true;
isShow = (this.viewer.isApplet () && !this.viewer.isSignedApplet () || !this.viewer.haveAccess (JV.Viewer.ACCESS.ALL) || this.viewer.getPathForAllFiles ().length > 0);
} else {
isCommand = false;
isShow = true;
...
var params;
var type = "SPT";
var tok = (isCommand && args.length == 1 ? 1073741884 : J.scriptext.CmdExt.tokAtArray (pt, args));
switch (tok) {
case 0:
break;
case 135271429:
...
type = "(image)";
}}if (J.scriptext.CmdExt.tokAtArray (pt, args) == 2) {
...
if (J.scriptext.CmdExt.tokAtArray (pt + 1, args) == 2 && J.scriptext.CmdExt.tokAtArray (pt + 2, args) == 2) {
width = J.script.SV.iValue (this.tokenAt (++pt, args));
height = J.script.SV.iValue (this.tokenAt (++pt, args));
}if (J.scriptext.CmdExt.tokAtArray (pt + 1, args) == 2) quality = J.script.SV.iValue (this.tokenAt (++pt, args));
...
}switch (J.scriptext.CmdExt.tokAtArray (pt, args)) {
case 0:
isShow = true;
break;
case 1073741884:
...
if (pt == argCount - 3 && J.scriptext.CmdExt.tokAtArray (pt + 1, args) == 1048583) {
...
}if (fileName.equalsIgnoreCase ("clipboard") || !this.viewer.haveAccess (JV.Viewer.ACCESS.ALL)) fileName = null;
break;
default:
this.invArg ();
}
...
data = type.intern ();
if (isExport) {
if (timeMsg) JW.Logger.startTimer ("export");
...
if (timeMsg) this.showString (JW.Logger.getTimerMsg ("export", 0));
} else {
msg = data;
}if (msg != null) {
...
data = (this.e.getParameter (J.script.SV.sValue (this.tokenAt (isCommand ? 2 : 1, args)), 1073742190)).asString ();
type = "TXT";
} else if (data === "SPT") {
if (isCoord) {
...
if (fileName != null && (bytes = data = this.viewer.createZip (fileName, type, scripts)) == null) this.e.evalError ("#CANCELED#", null);
...
if (timeMsg) JW.Logger.startTimer ("write");
if (doDefer) {
...
msg = this.viewer.processWriteOrCapture (params);
}if (timeMsg) this.showString (JW.Logger.getTimerMsg ("write", 0));
...
break;
case 135270423:
if (!this.chk) msg = JW.Escape.e (this.viewer.cacheList ());
break;
case 1073741915:
this.checkLength (2);
...
len = 3;
if (msg.length > 0) {
msg = this.viewer.getChemicalInfo (msg, this.getToken (2));
...
var pt2 = this.centerParameter (++this.e.iToken);
if (!this.chk) msg = this.viewer.getSymmetryOperation (null, 0, pt1, pt2, false);
len = ++this.e.iToken;
} else {
var iop = (this.e.checkLength23 () == 2 ? 0 : this.intParameter (2));
...
var vdwType = null;
if (this.slen > 2) {
vdwType = J.constant.EnumVdw.getVdwType (this.paramAsStr (2));
...
if (!this.chk) this.showString (this.viewer.getFunctionCalls (this.e.optParameterAsString (2)));
return;
case 1085443:
this.checkLength (2);
...
case 1074790760:
if ((len = this.slen) == 2) {
if (!this.chk) this.viewer.showUrl (this.e.getFullPathName ());
return;
...
break;
case 1073742192:
if (!this.chk) msg = this.viewer.getAtomDefs (this.e.definedAtomSets) + this.viewer.getVariableList () + this.getContext (true);
break;
case 536870926:
...
break;
case 536875070:
msg = this.viewer.showTimeout ((len = this.slen) == 2 ? null : this.paramAsStr (2));
break;
case 536870918:
value = JW.Escape.eP (this.viewer.getDefaultLattice ());
break;
case 4126:
...
break;
case 1073741828:
msg = JW.Escape.eAP (this.viewer.getAdditionalHydrogens (null, true, true, null));
break;
case 1613758476:
...
case 1048581:
if ((len = this.slen) == 2) {
if (!this.chk) msg = this.viewer.getCoordinateState (this.viewer.getSelectedAtoms ());
break;
...
if (!this.chk) msg = this.viewer.getEmbeddedFileState (this.paramAsStr (3));
break;
}len = 3;
...
break;
case 135270408:
var type = ((len = this.slen) == 3 ? this.paramAsStr (2) : null);
if (!this.chk) {
var data = (type == null ? this.lastData : this.viewer.getData (type));
msg = (data == null ? "no data" : JW.Escape.encapsulateData (data[0], data[1], (data[3]).intValue ()));
}break;
case 1073742152:
var info = null;
if ((len = this.slen) == 2) {
...
case 12289:
if (!this.chk) msg = "center " + JW.Escape.eP (this.viewer.getRotationCenter ());
break;
case 135176:
...
if (!this.chk) msg = this.viewer.getFileAsString (value, true);
break;
case 4115:
...
if (this.e.scriptLevel == 0) this.viewer.removeCommand ();
...
var v = this.e.getParameter (str, 1073742190);
if (tok == 1073741992) {
msg = v.toJSON ();
} else {
msg = v.escape ();
...
if (str.indexOf (" ") >= 0) this.showString (str);
 else this.showString (str + " = " + (this.e.getParameter (str, 1073742190)).escape ());
}}, $fz.isPrivate = true, $fz));
$_M(c$, "stereo", 
($fz = function () {
...
case 1073741824:
if (!degreesSeen) degrees = 3;
stereoMode = J.constant.EnumStereoMode.getStereoMode (this.paramAsStr (i));
...
if (this.viewer.modelSet.atoms[atomIndex].modelIndex != this.viewer.modelSet.modelCount - 1) return;
this.viewer.clearModelDependentObjects ();
var atomCount = this.viewer.modelSet.getAtomCount ();
if (pt == null) {
this.viewer.statusManager.modifySend (atomIndex, this.viewer.modelSet.atoms[atomIndex].modelIndex, 1, this.e.fullCommand);
this.viewer.modelSet.assignAtom (atomIndex, type, true);
if (!JU.PT.isOneOf (type, ";Mi;Pl;X;")) this.viewer.modelSet.setAtomNamesAndNumbers (atomIndex, -atomCount, null);
this.viewer.statusManager.modifySend (atomIndex, this.viewer.modelSet.atoms[atomIndex].modelIndex, -1, "OK");
this.viewer.refresh (3, "assignAtom");
return;
}var atom = this.viewer.modelSet.atoms[atomIndex];
var bs = JW.BSUtil.newAndSetBit (atomIndex);
var pts = [pt];
var vConnections =  new JU.List ();
vConnections.addLast (atom);
var modelIndex = atom.modelIndex;
this.viewer.statusManager.modifySend (atomIndex, modelIndex, 3, this.e.fullCommand);
try {
bs = this.viewer.addHydrogensInline (bs, vConnections, pts);
atomIndex = bs.nextSetBit (0);
this.viewer.modelSet.assignAtom (atomIndex, type, false);
} catch (ex) {
if (Clazz.exceptionOf (ex, Exception)) {
} else {
throw ex;
...
this.viewer.modelSet.setAtomNamesAndNumbers (atomIndex, -atomCount, null);
...
this.viewer.statusManager.modifySend (bondIndex, modelIndex, 2, this.e.fullCommand);
...
if (Clazz.exceptionOf (ex, Exception)) {
JW.Logger.error ("assignBond failed");
this.viewer.statusManager.modifySend (bondIndex, modelIndex, -2, "ERROR " + ex);
} else {
throw ex;
}
}
...
this.viewer.statusManager.modifySend (index, modelIndex, 2, this.e.fullCommand);
...
sb.append (J.script.ScriptError.getErrorLineMessage (context.functionName, context.scriptFileName, this.e.getLinenumber (context), context.pc, J.script.ScriptEval.statementAsString (this.viewer, context.statement, -9999, this.e.debugHigh)));
...
m.set (this.viewer);
var data = (fileName == null ? null : this.viewer.getFileAsString (fileName, false));
...
$_M(c$, "getColorTrans", 
($fz = function (eval, i, allowNone, ret) {
var translucentLevel = 3.4028235E38;
if (eval.theTok != 1766856708) --i;
switch (this.tokAt (i + 1)) {
case 603979967:
i++;
translucentLevel = (this.isFloatParameter (i + 1) ? eval.getTranslucentLevel (++i) : this.viewer.getFloat (570425354));
break;
case 1073742074:
i++;
translucentLevel = 0;
...
}, $fz.isPrivate = true, $fz), "J.script.ScriptEval,~N,~B,~A");
$_M(c$, "getCapSlabObject", 
($fz = function (i, isLcaoCartoon) {
if (i < 0) {
return JW.TempArray.getSlabWithinRange (i, 0);
}var eval = this.e;
var data = null;
...
if (eval.isColorParam (i + 1)) {
slabColix = Short.$valueOf (JW.C.getColixTranslucent3 (JW.C.getColix (eval.getArgbParam (i + 1)), slabTranslucency != 0, slabTranslucency));
i = eval.iToken;
} else {
slabColix = Short.$valueOf (JW.C.getColixTranslucent3 (1, slabTranslucency != 0, slabTranslucency));
...
case 1679429641:
eval.iToken = i + 1;
data = JW.BoxInfo.getCriticalPoints (this.viewer.getBoundBoxVertices (), null);
break;
case 1073741872:
case 1614417948:
eval.iToken = i + 1;
...
pts = JW.BoxInfo.getCriticalPoints (unitCell.getUnitCellVertices (), unitCell.getCartesianOffset ());
...
return JW.TempArray.getSlabObjectType (tok, data, !isSlab, colorData);
}, $fz.isPrivate = true, $fz), "~N,~B");
...
}}, $fz.isPrivate = true, $fz), "~N,~N,~N,~N,~B,~O,~N,JU.BS");
$_M(c$, "moCombo", 
...
if (sb != null) sb.append (" ").append (JW.Escape.escapeColor (color));
...
var nmax = (haveIndex ? 1 : JW.BSUtil.cardinalityOf (bs));
...
}str = JW.Txt.formatStringI (str, "#", (n + 1));
sout[n++] = str;
if (haveIndex) break;
}
...
$_M(c$, "parseDataArray", 
($fz = function (str, is3D) {
str = JW.Parser.fixDataString (str);
var lines = JW.Parser.markLines (str, '\n');
