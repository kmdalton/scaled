...
if (meshSurface.normals != null) meshSurface.normalCount = meshSurface.vertexCount;
...
var polygon = meshSurface.polygonIndexes[i];
faces[ipt++] = (meshSurface.haveQuads ? polygon : [polygon[0], polygon[1], polygon[2]]);
}
var data = JW.MeshSurface.newMesh (false, meshSurface.vertices, meshSurface.vertexCount, faces, meshSurface.normals, 0);
...
var retVal = this.initOutput (viewer, privateKey, g3d, params);
if (!retVal) {
...
this.mtlout = viewer.openExportChannel (privateKey, mtlFileName, true);
} catch (ex) {
...
$_M(c$, "outputCircle1", 
($fz = function (ptCenter, ptPerp, colix, radius) {
...
}, $fz.isPrivate = true, $fz), "JU.P3,JU.P3,~N,~N");
$_M(c$, "outputCone1", 
($fz = function (ptBase, ptTip, radius, colix) {
...
$_M(c$, "outputEllipse1", 
($fz = function (ptCenter, ptZ, ptX, ptY, colix) {
var data = J["export"].MeshData.getCircleData ();
var matrix =  new JU.M4 ();
this.addTexture (colix, null);
...
this.addMesh (name, data, matrix, matrix, colix, null, null);
return true;
}, $fz.isPrivate = true, $fz), "JU.P3,JU.P3,JU.P3,JU.P3,~N");
$_M(c$, "outputEllipsoid1", 
($fz = function (center, rx, ry, rz, a, colix) {
...
}, $fz.isPrivate = true, $fz), "JU.P3,~N,~N,~N,JU.A4,~N");
$_M(c$, "outputCylinder1", 
($fz = function (ptCenter, pt1, pt2, colix, endcaps, radius, ptX, ptY) {
...
this.addMesh (name, data, matrix, matrix, colix, null, null);
}, $fz.isPrivate = true, $fz), "JU.P3,JU.P3,JU.P3,~N,~N,~N,JU.P3,JU.P3");
$_M(c$, "outputTriangle1", 
($fz = function (pt1, pt2, pt3, colix) {
var data = J["export"].MeshData.getTriangleData (pt1, pt2, pt3);
this.addTexture (colix, null);
var name = "Triangle" + this.triangleNum++;
var matrix = JU.M4.newM4 (null);
this.addMesh (name, data, matrix, matrix, colix, null, null);
}, $fz.isPrivate = true, $fz), "JU.P3,JU.P3,JU.P3,~N");
$_M(c$, "addTexture", 
($fz = function (colix, name) {
...
$_M(c$, "addMesh", 
($fz = function (name, data, matrix, matrix1, colix, dim, bsValid) {
if (this.surfacesOnly) {
...
var nVertices = data.vertexCount;
...
}, $fz.isPrivate = true, $fz), "~S,JW.MeshSurface,JU.M4,JU.M4,~N,~A,JU.BS");
$_M(c$, "outputList", 
($fz = function (pts, nPts, m, prefix, bsValid) {
...
}, $fz.isPrivate = true, $fz), "~A,~N,JU.M4,~S,JU.BS");
$_M(c$, "outputFace1", 
($fz = function (face, map, map2) {
this.output ("f");
...
this.output ("\n");
}, $fz.isPrivate = true, $fz), "~A,~N,~A,~A");
$_M(c$, "createTextureFile", 
($fz = function (name, data, dim) {
this.debugPrint ("createTextureFile: " + name);
var colixes = (data.polygonColixes == null ? data.vertexColixes : data.polygonColixes);
...
}var nUsed = data.polygonIndexes.length;
if (nUsed <= 0) {
...
for (var i = 0; i < data.polygonIndexes.length; i++) {
var rgb;
if (data.polygonColixes == null) {
var face = data.polygonIndexes[i];
sum.set (0, 0, 0);
for (var iVertex, $iVertex = 0, $$iVertex = face; $iVertex < $$iVertex.length && ((iVertex = $$iVertex[$iVertex]) || true); $iVertex++) sum.add (JU.CU.colorPtFromInt2 (this.g3d.getColorArgbOrGray (colixes[iVertex])));
...
fname = this.fileName = this.viewer.outputToFile (params);
...
}, $fz.isPrivate = true, $fz), "~S,JW.MeshSurface,~A");
