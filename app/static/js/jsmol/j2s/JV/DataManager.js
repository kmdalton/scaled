...
Clazz.load (["J.api.JmolDataManager", "java.util.Hashtable"], "JV.DataManager", ["java.lang.Boolean", "JU.AU", "$.BS", "$.PT", "$.SB", "J.constant.EnumVdw", "J.script.T", "JW.BSUtil", "$.Elements", "$.Escape", "$.Logger", "$.Parser"], function () {
...
JW.Parser.parseFloatArrayFromMatchAndField (stringData, this.viewer.bsUserVdws, 1, 0, data[2], 2, 0, this.viewer.userVdws, 1);
...
JW.Parser.parseFloatArrayBsData (strData == null ? JU.PT.getTokens (stringData) : strData, bs, f);
}} else if (matchField <= 0) {
bs = data[2];
JW.Parser.parseFloatArrayFromMatchAndField (stringData, bs, 0, 0, null, field, fieldColumnCount, f, 1);
} else {
var iData = data[2];
JW.Parser.parseFloatArrayFromMatchAndField (stringData, null, matchField, matchFieldColumnCount, iData, field, fieldColumnCount, f, 1);
bs =  new JU.BS ();
...
if (tok == 0) {
JW.Logger.error ("Unknown atom property: " + type);
return;
...
JW.BSUtil.deleteBits (obj[2], bsDeleted);
...
var i1 = (isAll ? JW.Elements.elementNumberMax : bs.length ());
for (var i = i0; i < i1 && i >= 0; i = (isAll ? i + 1 : bs.nextSetBit (i + 1))) sb.appendI (i).appendC ('\t').appendF (type === J.constant.EnumVdw.USER ? this.viewer.userVdws[i] : JW.Elements.getVanderwaalsMar (i, type) / 1000).appendC ('\t').append (JW.Elements.elementSymbolFromNumber (i)).appendC ('\n');
...
sb.append ("\n").append (JW.Escape.encapsulateData (name, data, 0));
}} else if (name.indexOf ("data2d") == 0) {
var obj = this.dataValues.get (name);
var data = obj[1];
if (data != null && (obj[3]).intValue () == 2) {
haveData = true;
sb.append ("\n").append (JW.Escape.encapsulateData (name, data, 2));
}} else if (name.indexOf ("data3d") == 0) {
var obj = this.dataValues.get (name);
var data = obj[1];
if (data != null && (obj[3]).intValue () == 3) {
haveData = true;
sb.append ("\n").append (JW.Escape.encapsulateData (name, data, 3));
}}}
return haveData;
