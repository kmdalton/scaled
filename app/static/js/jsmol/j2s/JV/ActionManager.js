...
Clazz.load (["javajs.api.EventManager", "JW.Rectangle", "JV.MouseState"], "JV.ActionManager", ["java.lang.Character", "$.Float", "JU.P3", "$.PT", "J.api.Interface", "J.i18n.GT", "J.thread.HoverWatcherThread", "JW.BSUtil", "$.Escape", "$.Logger", "$.Point3fi", "JV.binding.Binding", "$.JmolBinding"], function () {
...
this.rectRubber =  new JW.Rectangle ();
});
$_M(c$, "setViewer", 
...
var isLabel = (this.getAtomPickingMode () == 2 && this.isBound (JV.binding.Binding.getMouseAction (this.clickedCount, this.moved.modifiers, 1), 10));
...
$_M(c$, "getBindingInfo", 
function (qualifiers) {
return this.binding.getBindingInfo (JV.ActionManager.actionInfo, JV.ActionManager.actionNames, qualifiers);
}, "~S");
$_M(c$, "setBinding", 
function (newBinding) {
...
function () {
var script = ";set modelkitMode " + this.viewer.getBoolean (603979883) + ";set picking " + JV.ActionManager.getPickingModeName (this.atomPickingMode);
...
case 2:
if (!this.binding.name.equals ("extendedSelect")) this.setBinding (this.pfaatBinding == null ? this.pfaatBinding = JV.binding.Binding.newBinding ("Pfaat") : this.pfaatBinding);
break;
case 3:
if (!this.binding.name.equals ("drag")) this.setBinding (this.dragBinding == null ? this.dragBinding = JV.binding.Binding.newBinding ("Drag") : this.dragBinding);
this.rubberbandSelectionMode = true;
break;
case 1:
if (!this.binding.name.equals ("selectOrToggle")) this.setBinding (this.rasmolBinding == null ? this.rasmolBinding = JV.binding.Binding.newBinding ("Rasmol") : this.rasmolBinding);
break;
default:
...
if (JW.Logger.debuggingHigh && mode != 0) this.viewer.showString ("mouse action: " + mode + " " + buttonMods + " " + JV.binding.Binding.getMouseActionName (JV.binding.Binding.getMouseAction (count, buttonMods, mode), false), false);
switch (mode) {
case 0:
...
this.viewer.setFocus ();
if (this.atomPickingMode != 9 && this.isBound (JV.binding.Binding.getMouseAction (1, buttonMods, 4), 31)) return;
...
var isSelectAndDrag = this.isBound (JV.binding.Binding.getMouseAction (1, buttonMods, 4), 31);
if (buttonMods != 0) {
...
if (JW.Logger.debugging) JW.Logger.debug (JV.binding.Binding.getMouseActionName (this.pressAction, false));
...
if (buttonmods != 0) {
var newAction = this.viewer.notifyMouseClicked (x, y, JV.binding.Binding.getMouseAction (this.pressedCount, buttonmods, mode), mode);
...
case 27:
case 30:
bs = this.viewer.getAtomBits (1095761936, JW.BSUtil.newAndSetBit (this.dragAtomIndex));
...
var iatom = this.viewer.getSelectedAtoms ().nextSetBit (0);
if (iatom < 0) return;
...
if (dragRelease) {
this.viewer.notifyMouseClicked (x, y, JV.binding.Binding.getMouseAction (this.pressedCount, 0, 5), 5);
}if (this.drawMode && (this.isBound (this.dragAction, 8) || this.isBound (this.dragAction, 9)) || this.labelMode && this.isBound (this.dragAction, 10)) {
...
}if (JW.Logger.debugging) JW.Logger.debug (JV.binding.Binding.getMouseActionName (this.clickAction, false));
...
if (iatom >= 0) script = JU.PT.rep (script, "_POINT", JW.Escape.eP (this.viewer.getModelSet ().atoms[iatom]));
...
script = JU.PT.rep (script, "_POINT", JW.Escape.eP (nearestPoint));
script = JU.PT.rep (script, "_OBJECT", JW.Escape.escapeMap (t));
...
}, $fz.isPrivate = true, $fz), "~N,~N,JW.Point3fi,~B");
$_M(c$, "isSelectAction", 
($fz = function (action) {
...
this.viewer.setPendingMeasurement (this.measurementPending = this.getMP ());
this.measurementQueued = this.measurementPending;
}, $fz.isPrivate = true, $fz), "~N");
$_M(c$, "getMP", 
($fz = function () {
return (J.api.Interface.getInterface ("JM.MeasurementPending")).set (this.viewer.modelSet);
}, $fz.isPrivate = true, $fz));
...
}, $fz.isPrivate = true, $fz), "~N,JW.Point3fi,~B");
...
var bs = (this.viewer.getMotionFixedAtoms ().cardinality () == 0 ? this.viewer.getAtomBits ((this.viewer.isAtomPDB (iAtom) ? 1087373318 : 1095761936), JW.BSUtil.newAndSetBit (iAtom)) : JW.BSUtil.setAll (this.viewer.getAtomCount ()));
...
return n;
}, $fz.isPrivate = true, $fz), "~N,JW.Point3fi");
$_M(c$, "setMotion", 
...
this.runScript ("zoomTo " + JW.Escape.eP (ptClicked));
}return;
case 5:
case 6:
...
this.viewer.setStatusAtomPicked (atomIndex, "inverted: " + JW.Escape.eBS (bs), null);
}return;
case 7:
if (this.isBound (this.clickAction, 4)) {
bs = JW.BSUtil.newAndSetBit (atomIndex);
this.viewer.deleteAtoms (bs, false);
this.viewer.setStatusAtomPicked (atomIndex, "deleted: " + JW.Escape.eBS (bs), null);
}return;
}
...
this.runScript ("assign atom ({" + this.dragAtomIndex + "}) \"" + this.pickAtomAssignType + "\" " + JW.Escape.eP (ptNew));
}}}this.exitMeasurementMode (null);
...
break;
case 8:
this.viewer.deleteBonds (JW.BSUtil.newAndSetBit (index));
}
}, $fz.isPrivate = true, $fz), "~N");
...
}, $fz.isPrivate = true, $fz), "JW.Point3fi,~N");
$_M(c$, "reset", 
($fz = function () {
...
var e = ((c.x - d.x)) / this.b$["JV.ActionManager"].viewer.getScreenWidth () * 360;
var f = ((c.y - d.y)) / this.b$["JV.ActionManager"].viewer.getScreenHeight () * 360;
