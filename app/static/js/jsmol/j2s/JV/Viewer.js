...
Clazz.load (["java.lang.Enum", "javajs.api.PlatformViewer", "J.api.JmolViewer", "J.atomdata.AtomDataServer", "java.util.Hashtable", "javajs.awt.Dimension", "JU.List", "J.atomdata.RadiusData", "J.constant.EnumVdw", "J.i18n.GT", "JW.CommandHistory"], "JV.Viewer", ["java.io.Reader", "java.lang.Boolean", "$.Character", "$.Float", "JU.BS", "$.CU", "$.DF", "$.P3", "$.P3i", "$.PT", "$.SB", "$.V3", "J.adapter.smarter.SmarterJmolAdapter", "J.api.Interface", "$.JmolAppConsoleInterface", "J.constant.EnumAnimationMode", "$.EnumAxesMode", "$.EnumFileStatus", "$.EnumStereoMode", "J.io.CifDataReader", "$.JmolBinary", "JM.Group", "J.script.SV", "$.T", "J.thread.TimeoutThread", "JW.BSUtil", "$.C", "$.Elements", "$.Escape", "$.GData", "$.JmolMolecule", "$.Logger", "$.Measure", "$.Parser", "$.TempArray", "$.Txt", "JV.ActionManager", "$.AnimationManager", "$.ColorManager", "$.FileManager", "$.JC", "$.ModelManager", "$.SelectionManager", "$.ShapeManager", "$.StateManager", "$.StatusManager", "$.TransformManager", "JV.binding.Binding"], function () {
...
$_M(c$, "finalize", 
function () {
if (JW.Logger.debugging) JW.Logger.debug ("viewer finalize " + this);
...
this.stateManager =  new JV.StateManager (this);
this.colorManager =  new JV.ColorManager (this, this.gdata);
this.statusManager =  new JV.StatusManager (this);
this.tm =  new JV.TransformManager (this, 2147483647, 0);
this.selectionManager =  new JV.SelectionManager (this);
if (this.haveDisplay) {
...
}this.modelManager =  new JV.ModelManager (this);
this.shapeManager =  new JV.ShapeManager (this);
this.tempArray =  new JW.TempArray ();
this.animationManager =  new JV.AnimationManager (this);
...
this.fileManager =  new JV.FileManager (this);
this.definedAtomSets =  new java.util.Hashtable ();
this.setJmolStatusListener (statusListener);
if (this.$isApplet) {
JW.Logger.info ("viewerOptions: \n" + JW.Escape.escapeMap (this.viewerOptions));
...
this.fileManager.setAppletContext (JV.Viewer.appletDocumentBase);
...
$_M(c$, "getDataManager", 
($fz = function () {
return (this.dataManager == null ? (this.dataManager = (J.api.Interface.getInterface ("JV.DataManager")).set (this)) : this.dataManager);
}, $fz.isPrivate = true, $fz));
$_M(c$, "getScriptManager", 
($fz = function () {
if (this.allowScripting && this.scriptManager == null) {
this.scriptManager = J.api.Interface.getOption ("script.ScriptManager");
if (this.scriptManager == null) {
this.allowScripting = false;
return null;
}this.eval = this.scriptManager.setViewer (this);
...
function () {
return (this.haveAccess (JV.Viewer.ACCESS.ALL) ? this.global.getParameter ("exportDrivers") : "");
});
$_M(c$, "getHtmlName", 
function () {
...
function () {
this.modelSet.saveModelOrientation (this.getCurrentModelIndex (), this.stateManager.getOrientation ());
});
$_M(c$, "getOrientation", 
function () {
...
this.stateManager.saveContext (saveName, sc);
this.setUserVariable (saveName, J.script.SV.newV (14, sc));
return sc;
}, "~S");
$_M(c$, "getContext", 
function (saveName) {
return this.stateManager.getContext (saveName);
}, "~S");
$_M(c$, "restoreBonds", 
function (saveName) {
...
$_M(c$, "deleteSaved", 
function (name) {
this.stateManager.deleteSaved (name);
}, "~S");
$_M(c$, "getSavedState", 
function (saveName) {
...
$_M(c$, "saveSelection", 
function (saveName) {
this.stateManager.saveSelection (saveName, this.getSelectedAtoms ());
...
if (this.modelSet.setRotationRadius (this.getCurrentModelIndex (), angstroms)) this.global.setF ("rotationRadius", angstroms);
}, "~N,~B");
...
this.gdata.setZShade (this.tm.zShadeEnabled, this.tm.zSlabValue, this.tm.zDepthValue, this.global.zShadePower);
});
$_M(c$, "rotatePoint", 
function (pt, ptRot) {
...
case 1112541207:
case 135270418:
return this.modelSet.getBoundBoxOrientation (type, this.getSelectedAtoms ());
case 1073742035:
...
this.global.setS ("=color " + JW.Elements.elementNameFromNumber (elementNumber), JW.Escape.escapeColor (argb));
...
this.global.setS (name + "Color", JW.Escape.escapeColor (argb));
}, "~S,~N");
...
JW.BSUtil.andNot (bs, this.selectionManager.getDeletedAtoms ());
return bs;
...
function () {
return this.selectionManager.getSelectedAtomsNoSubset ();
});
...
this.modelSet.setFormalCharges (this.getSelectedAtoms (), formalCharge);
}, "~N");
...
this.setErrorMessage (null, null);
try {
var script = this.modelManager.createAtomDataSet (atomSetCollection, tokType);
switch (tokType) {
case 1146095626:
...
return this.fileManager.getFileDataOrErrorAsString (data, -1, false, allowBinary, !allowBinary);
}, "~A,~B");
$_M(c$, "getFilePath", 
...
if (bsAtoms == null) bsAtoms = this.getSelectedAtoms ();
return this.modelSet.calculateStructures (bsAtoms, asDSSP, !this.isAnimationOn (), this.global.dsspCalcHydrogen, setStructure);
}, "JU.BS,~B,~B");
...
$_M(c$, "getMinimizer", 
function (createNew) {
return (this.minimizer == null && createNew ? (this.minimizer = J.api.Interface.getOption ("minimize.Minimizer")).setProperty ("viewer", this) : this.minimizer);
}, "~B");
...
function (modelIndex) {
return this.excludeAtoms (this.modelSet.getModelAtomBitSetIncludingDeleted (modelIndex, true), false);
}, "~N");
$_M(c$, "getModelBitSet", 
...
function (bsModels) {
return this.excludeAtoms (this.modelSet.getModelAtomBitSetIncludingDeletedBs (bsModels), false);
}, "JU.BS");
$_M(c$, "excludeAtoms", 
...
function () {
return this.modelSet.getBoundBoxCenter (this.getCurrentModelIndex ());
});
...
function (center) {
return this.modelSet.calcRotationRadius (this.getCurrentModelIndex (), center);
}, "JU.P3");
...
return this.modelSet.getSymmetryInfoString (this.getCurrentModelIndex (), spaceGroup, symop, pt1, pt2, null, labelOnly);
}, "~S,~N,JU.P3,JU.P3,~B");
...
function (bsAtoms) {
return this.modelSet.getBondsForSelectedAtoms (bsAtoms, this.global.bondModeOr || JW.BSUtil.cardinalityOf (bsAtoms) == 1);
}, "JU.BS");
$_M(c$, "frankClicked", 
function (x, y) {
...
function (points) {
this.modelSet.setModelCage (this.getCurrentModelIndex (), J.api.Interface.getSymmetry ().getUnitCell (points, true));
}, "~A");
...
return JW.JmolMolecule.getBranchBitSet (this.modelSet.atoms, atomIndex, this.getModelUndeletedAtomsBitSet (this.modelSet.atoms[atomIndex].modelIndex), null, atomIndexNot, allowCyclic, true);
}, "~N,~N,~B");
...
function () {
this.modelSet.calcSelectedGroupsCount (this.getSelectedAtoms ());
});
$_M(c$, "calcSelectedMonomersCount", 
function () {
this.modelSet.calcSelectedMonomersCount (this.getSelectedAtoms ());
});
$_M(c$, "calcSelectedMoleculesCount", 
function () {
this.modelSet.calcSelectedMoleculesCount (this.getSelectedAtoms ());
});
$_M(c$, "getFileHeader", 
function () {
return this.modelSet.getFileHeader (this.getCurrentModelIndex ());
});
$_M(c$, "getFileData", 
function () {
return this.modelSet.getFileData (this.getCurrentModelIndex ());
});
$_M(c$, "getCifData", 
function (modelIndex) {
...
$_M(c$, "getPDBHeader", 
function () {
return this.modelSet.getPDBHeader (this.getCurrentModelIndex ());
});
$_M(c$, "getChimeInfo", 
function (tok) {
...
function () {
return this.modelSet.getProteinStructureState (this.getSelectedAtoms (), false, false, 3);
});
...
if (bs != null && this.global.rangeSelected) bs.and (this.getSelectedAtoms ());
...
function () {
return this.modelSet.setConformation (this.getSelectedAtoms ());
});
...
if (m >= 0) return this.modelSet.getUnitCell (m);
...
function () {
return this.modelSet.selectDisplayedTrajectories (JW.BSUtil.copy (this.animationManager.bsVisibleModels));
});
$_M(c$, "isAnimationOn", 
function () {
...
this.tm.setFrameOffsets (this.frameOffsets = this.modelSet.getFrameOffsets (this.bsFrameOffsets));
}, "JU.BS");
...
}this.tm.setScreenParameters (width, height, isImageWrite || isReset ? this.global.zoomLarge : false, this.antialiasDisplay, false, false);
...
$_M(c$, "beginRendering", 
($fz = function (isDouble, isImageWrite) {
this.gdata.beginRendering (this.tm.getStereoRotationMatrix (isDouble), this.global.translucent, isImageWrite, !this.checkMotionRendering (603979967));
}, $fz.isPrivate = true, $fz), "~B,~B");
$_M(c$, "isAntialiased", 
function () {
...
this.finalizeTransformParameters ();
var minMax = this.shapeManager.finalizeAtoms (this.tm.bsSelectedAtoms, this.tm.ptOffset);
...
break;
case 'S':
format = this.global.nihResolverFormat + "/stdinchikey";
break;
case 'T':
format = this.global.nihResolverFormat + "/stdinchi";
break;
...
case '_':
var server = JV.FileManager.fixFileNameVariables (this.global.edsUrlFormat, f);
var strCutoff = JV.FileManager.fixFileNameVariables (this.global.edsUrlCutoff, f);
...
this.global.setS ("picking", JV.ActionManager.getPickingModeName (this.actionManager.getAtomPickingMode ()));
...
this.global.setS ("pickingStyle", JV.ActionManager.getPickingStyleName (this.actionManager.getPickingStyle ()));
}, "~S,~N");
$_M(c$, "getDrawHover", 
function () {
...
var currentFrame = this.animationManager.currentModelIndex;
var fileNo = frameID;
...
}var currentMorphModel = this.animationManager.currentMorphModel;
this.global.setI ("_currentFrame", currentFrame);
this.global.setI ("_morphCount", this.animationManager.morphCount);
this.global.setF ("_currentMorphFrame", currentMorphModel);
...
}this.statusManager.setStatusFrameChanged (fileNo, modelNo, (this.animationManager.animationDirection < 0 ? -firstNo : firstNo), (this.animationManager.currentDirection < 0 ? -lastNo : lastNo), currentFrame, currentMorphModel, entryName);
...
info = (info.length == 0 ? this.getAtomInfoXYZ (atomIndex, this.global.messageStyleChime) : this.modelSet.getAtomInfo (atomIndex, info));
}this.global.setPicked (atomIndex);
this.global.setS ("_pickinfo", info);
this.statusManager.setStatusAtomPicked (atomIndex, info, map);
...
}JW.Logger.error ("viewer.getBooleanProperty(" + key + ") - unrecognized");
return false;
}, "~S");
$_V(c$, "getInt", 
function (tok) {
...
case 553648138:
value = (value == 0 ? 0 : 1);
this.global.bondingVersion = JW.Elements.bondingVersion = value;
break;
case 553648137:
...
break;
case 603979885:
this.global.multiProcessor = value && (JV.Viewer.nProcessors > 1);
break;
case 603979884:
...
this.tm.setZoomHeight (this.global.zoomHeight, value);
break;
case 603979982:
this.global.zoomHeight = value;
this.tm.setZoomHeight (value, this.global.zoomLarge);
break;
case 603979873:
...
function () {
if (!this.global.navigationMode || !this.tm.canNavigate ()) return false;
...
($fz = function () {
this.tm.setCameraDepthPercent (this.global.defaultCameraDepth, true);
this.tm.setPerspectiveDepth (this.global.defaultPerspectiveDepth);
this.tm.setStereoDegrees (-5);
this.tm.setVisualRange (this.global.visualRange);
this.tm.setSpinOff ();
this.tm.setVibrationPeriod (0);
...
$_M(c$, "isStereoDouble", 
function () {
return this.tm.stereoMode === J.constant.EnumStereoMode.DOUBLE;
});
...
this.setNavOn (false);
return;
}this.tm.rotateAboutPointsInternal (null, pt1, pt2, this.global.pickingSpinRate, 3.4028235E38, isClockwise, true, null, false, null, null, null);
}, "JU.P3,JU.P3,~B");
$_M(c$, "getModelDipole", 
function () {
...
function (p) {
if (!Float.isNaN (p.x + p.y + p.z)) this.global.ptDefaultLattice.setT (p);
this.global.setS ("defaultLattice", JW.Escape.eP (p));
}, "JU.P3");
...
$_M(c$, "getPdbAtomData", 
function (bs, sb) {
return this.modelSet.getPdbAtomData (bs == null ? this.getSelectedAtoms () : bs, sb);
}, "JU.BS,JU.OC");
...
function () {
return this.modelSet.isJmolDataFrameForModel (this.getCurrentModelIndex ());
});
...
this.modelSet.setFrameTitle (JW.BSUtil.newAndSetBit (modelIndex), title);
}, "~N,~S");
...
$_M(c$, "getFrameTitle", 
function () {
return this.modelSet.getFrameTitle (this.getCurrentModelIndex ());
});
...
$_M(c$, "setProteinType", 
function (type, bs) {
this.modelSet.setProteinType (bs == null ? this.getSelectedAtoms () : bs, type);
...
function (i) {
return (this.defaultVdw === J.constant.EnumVdw.USER ? this.userVdwMars[i] : JW.Elements.getVanderwaalsMar (i, this.defaultVdw));
}, "~N");
...
this.statusManager.modifySend (-1, modelIndex, 2, "delete bonds " + JW.Escape.eBond (bsDeleted));
...
this.statusManager.modifySend (-1, modelIndex, 1, "delete atoms " + JW.Escape.eBS (bsDeleted));
...
function () {
return this.modelSet.calculatePointGroup (this.getSelectedAtoms ());
});
...
return this.modelSet.getPointGroupAsString (this.getSelectedAtoms (), asDraw, type, index, scale);
}, "~B,~S,~N,~N");
$_M(c$, "loadImage", 
...
JW.Logger.error ("viewer handling error condition: " + er + "  ");
...
function (bs1, bs2) {
return this.modelSet.calculateStruts (bs1 == null ? this.getSelectedAtoms () : bs1, bs2 == null ? this.getSelectedAtoms () : bs2);
}, "JU.BS,JU.BS");
...
var bsMotionFixed = JW.BSUtil.copy (bsFixed == null ? this.selectionManager.getMotionFixedAtoms () : bsFixed);
...
return this.modelSet.getPdbData (modelIndex, type, this.getSelectedAtoms (), parameters, null);
}, "~N,~S,~A");
...
info = JV.JC.getPdbBondInfo (JM.Group.lookupGroupID (group3), this.global.legacyHAddition);
...
if (this.getScriptManager () == null) return null;
return this.scriptManager.addHydrogensInline (bsAtoms, vConnections, pts);
}, "JU.BS,JU.List,~A");
...
var isOK = this.getScriptManager () != null && this.eval.evalParallel (context, (shapeManager == null ? this.shapeManager : shapeManager));
this.displayLoadErrors = true;
return isOK;
...
if (isQ) this.global.setS ("_modt", JW.Escape.eP (t1));
...
function (tok) {
if (!this.getInMotion (true) && !this.tm.spinOn && !this.tm.vibrationOn && !this.animationManager.animationOn) return true;
...
{
{
self.Jmol && Jmol.extend && Jmol.extend("viewer", JV.Viewer.prototype);
