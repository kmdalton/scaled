...
Clazz.load (null, "JV.OutputManager", ["java.lang.Boolean", "java.util.Date", "$.Hashtable", "$.Map", "JU.List", "$.PT", "$.SB", "J.api.Interface", "J.i18n.GT", "J.io.JmolBinary", "JW.Logger", "$.Txt", "JV.FileManager", "$.JC", "$.Viewer"], function () {
...
JW.Logger.errorEx ("IO Exception", exc);
return exc.toString ();
} else {
throw exc;
}
...
JW.Logger.error ("state could not be saved: " + e.toString ());
...
var ie = J.api.Interface.getOption ("image." + type + "Encoder");
if (ie == null) {
...
if (!this.viewer.haveAccess (JV.Viewer.ACCESS.ALL)) return null;
if (fileName != null) {
...
var sb =  new JU.SB ();
if (bsFrames == null) {
this.viewer.tm.vibrationOn = true;
sb =  new JU.SB ();
for (var i = 0; i < nVibes; i++) {
for (var j = 0; j < 20; j++) {
this.viewer.tm.setVibrationT (j / 20 + 0.2501);
...
var msg = (type.equals ("PDB") || type.equals ("PQR") ? this.viewer.getPdbAtomData (null, out) : type.startsWith ("PLOT") ? this.viewer.modelSet.getPdbData (modelIndex, type.substring (5), this.viewer.getSelectedAtoms (), parameters, out) : getCurrentFile ? out.append (this.viewer.getCurrentFileAsString ()).toString () : this.viewer.getFileAsBytes (pathName, out));
out.closeChannel ();
...
fileName = JV.FileManager.getLocalPathForWritingFile (this.viewer, fileName);
...
var height = JV.OutputManager.getInt (params, "height", 0);
var quality = JV.OutputManager.getInt (params, "quality", -2147483648);
var captureMode = JV.OutputManager.getInt (params, "captureMode", -2147483648);
...
JW.Logger.error (this.viewer.setErrorMessage (sret = "ERROR creating image??: " + er, null));
} finally {
this.viewer.creatingImage = false;
...
path = this.getLogPath (logFilePath + value);
 ...
 else JW.Logger.info (J.i18n.GT.o (J.i18n.GT._ ("Setting log file to {0}"), path));
if (value == null || !this.viewer.haveAccess (JV.Viewer.ACCESS.ALL)) {
JW.Logger.info (J.i18n.GT._ ("Cannot set log file path."));
value = null;
} else {
...
JW.Logger.info (data);
return;
}var out = (this.viewer.haveAccess (JV.Viewer.ACCESS.ALL) ? this.openOutputChannel (this.privateKey, this.viewer.logFileName, true, !doClear) : null);
if (!doClear) {
...
if (Clazz.exceptionOf (e, Exception)) {
if (JW.Logger.debugging) JW.Logger.debug ("cannot log " + data);
} else {
throw e;
}
}
...
var isLocal = !this.viewer.isJS && JV.FileManager.isLocal (name);
var newName = name;
...
if (!sceneScriptOnly) {
script = JW.Txt.replaceQuotedStrings (script, fileNames, newFileNames);
v.addLast ("state.spt");
v.addLast (null);
...
v.addLast (null);
script = JW.Txt.replaceQuotedStrings (scripts[1], fileNames, newFileNames);
v.addLast (script.getBytes ());
...
JW.Logger.info ("creating zip file " + (outFileName == null ? "" : outFileName) + "...");
var fileList = "";
try {
var bos;
{
...
JW.Logger.info ("duplicate entry");
continue;
}fileList += key;
...
JW.Logger.info ("...added " + fname + " (" + nOut + " bytes)");
}
zos.flush ();
zos.close ();
JW.Logger.info (nBytesOut + " bytes prior to compression");
...
}, $fz.isPrivate = true, $fz), "~N,JV.FileManager,JV.Viewer,JU.OC,JU.List,~S");
