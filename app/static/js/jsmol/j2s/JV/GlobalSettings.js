...
Clazz.load (["java.util.Hashtable", "JU.P3", "J.constant.EnumAxesMode", "$.EnumCallback"], "JV.GlobalSettings", ["java.lang.Boolean", "$.Float", "JU.BS", "$.DF", "$.List", "J.constant.EnumStructure", "J.script.SV", "JW.BSUtil", "$.Escape", "$.Logger", "$.Txt", "JV.JC", "$.StateManager", "$.Viewer"], function () {
...
this.setI ("logLevel", JW.Logger.getLogLevel ());
...
}, $fz.isPrivate = true, $fz), "~S,JV.GlobalSettings");
$_M(c$, "setB", 
function (name, value) {
...
JW.Logger.info ("all user-defined variables deleted");
} else if (this.htUserVariables.containsKey (key)) {
JW.Logger.info ("variable " + key + " deleted");
...
return JV.StateManager.varClip (name, JW.Escape.e (v), nMax);
...
if (p.tok != 10 || !(p.value).get (atomIndex)) pickedList.pushPop (J.script.SV.newV (10, JW.BSUtil.newAndSetBit (atomIndex)));
}, "~N");
...
}return (format.indexOf ("%FILE") < 0 ? format + id : JW.Txt.formatStringS (format, "FILE", id));
}, "~S,~S");
