...
Clazz.load (["JU.A4", "$.M3", "$.M4", "$.P3", "$.P3i", "$.V3", "J.constant.EnumStereoMode", "JW.Point3fi"], "JV.TransformManager", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.P4", "$.Quat", "$.SB", "J.api.Interface", "JW.Escape", "$.Logger"], function () {
...
this.ptVibTemp =  new JW.Point3fi ();
...
}, $fz.isPrivate = true, $fz), "JV.Viewer,~N,~N");
$_M(c$, "homePosition", 
function (resetSpin) {
...
if (this.mode == 1) {
info.put ("navigationCenter", "navigate center " + JW.Escape.eP (this.navigationCenter));
...
this.matrixTemp.setToM3 (this.matrixRotate);
...
}this.viewer.global.setS ("_slabPlane", JW.Escape.eP4 (this.getSlabDepthPlane (false)));
this.viewer.global.setS ("_depthPlane", JW.Escape.eP4 (this.getSlabDepthPlane (true)));
...
this.movetoThread.setManager (this, this.viewer, [dRot, dTrans, [dZoom, dSlab, floatSecondsTotal, fps]]);
...
var nSteps = this.movetoThread.setManager (this, this.viewer, [center, matrixEnd, navCenter, [floatSecondsTotal, zoom, xTrans, yTrans, newRotationRadius, pixelScale, navDepth, xNav, yNav, cameraDepth, cameraX, cameraY]]);
if (nSteps <= 0 || this.viewer.global.waitForMoveTo) {
if (nSteps > 0) this.movetoThread.setEval (eval);
this.movetoThread.run ();
if (!this.viewer.isSingleThreaded) this.movetoThread = null;
} else {
...
JV.TransformManager.truncate2 (sb, this.zoomPercentSetting);
JV.TransformManager.truncate2 (sb, this.getTranslationXPercent ());
JV.TransformManager.truncate2 (sb, this.getTranslationYPercent ());
sb.append (" ");
...
$_M(c$, "getCenterText", 
($fz = function () {
return JW.Escape.eP (this.fixedRotationCenter);
}, $fz.isPrivate = true, $fz));
...
sb.append (" zoom");
JV.TransformManager.truncate2 (sb, this.zoomPercent);
sb.append (";");
...
sb.append ("navigate 0 center ").append (JW.Escape.eP (this.navigationCenter));
...
if (Float.isNaN (z)) {
if (!this.haveNotifiedNaN && JW.Logger.debugging) JW.Logger.debug ("NaN seen in TransformPoint");
...
if (JW.Logger.debugging) JW.Logger.debug ("NaN found in transformPoint ");
this.haveNotifiedNaN = true;
...
this.getNavigationOffset ();
return s + JW.Escape.eP (this.navigationCenter) + " " + this.getNavigationOffsetPercent ('X') + " " + this.getNavigationOffsetPercent ('Y') + " " + this.getNavigationDepthPercent ();
}, "~B");
