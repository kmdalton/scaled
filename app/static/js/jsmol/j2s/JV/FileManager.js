...
Clazz.load (["javajs.api.BytePoster", "java.util.Hashtable"], "JV.FileManager", ["java.io.BufferedInputStream", "$.BufferedReader", "java.lang.Boolean", "java.net.URL", "$.URLEncoder", "JU.AU", "$.Base64", "$.List", "$.PT", "$.SB", "J.api.Interface", "J.io.FileReader", "$.JmolBinary", "JW.Logger", "$.Txt", "JV.Viewer"], function () {
...
JW.Logger.info ("FileManager.getAtomSetCollectionFromStrings(string[])");
...
JW.Logger.info ("FileManager.getAtomSetCollectionFromArrayData(Vector)");
...
var fr = J.api.Interface.getOption ("io2.FilesReader");
...
var dr = J.api.Interface.getOption ("io2." + reader + "DataReader");
return dr.setData (data);
}, $fz.isPrivate = true, $fz), "~O");
$_M(c$, "createAtomSetCollectionFromDOM", 
function (DOMNode, htParams) {
var aDOMReader = J.api.Interface.getOption ("io2.DOMReadaer");
...
name = url.toString ();
if (showMsg && name.toLowerCase ().indexOf ("password") < 0) JW.Logger.info ("FileManager opening 1 " + name);
...
if (showMsg) JW.Logger.info ("FileManager opening 2 " + name);
...
if (bytes == null) JW.Logger.info ("FileManager opening 3 " + name);
name = subFileList[0];
...
var doc = J.api.Interface.getOption ("io2.CompoundDocument");
...
var bd = J.api.Interface.getOption ("io2.BinaryDocument");
...
if (name.indexOf (":") < 0 && name.indexOf ("/") != 0) name = JV.FileManager.addDirectory (this.viewer.getDefaultDirectory (), name);
if (this.appletDocumentBaseURL == null) {
if (JV.FileManager.urlTypeIndex (name) >= 0 || this.viewer.haveAccess (JV.Viewer.ACCESS.NONE) || this.viewer.haveAccess (JV.Viewer.ACCESS.READSPT) && !name.endsWith (".spt") && !name.endsWith ("/")) {
try {
...
JW.Logger.info ("FileManager substituting " + name0 + " --> " + names[0]);
...
JV.FileManager.setLocalPath (this.viewer, path, true);
}}return names;
...
var dir = null;
try {
dir = JV.FileManager.getLocalDirectory (viewer, false);
} catch (e) {
if (Clazz.exceptionOf (e, Exception)) {
} else {
throw e;
...
}}JW.Logger.info ("FileManager substituting " + name0 + " --> " + name);
oldFileNames.addLast ("\"" + name0 + "\"");
newFileNames.addLast ("\1\"" + name + "\"");
}
return JW.Txt.replaceStrings (script, oldFileNames, newFileNames);
...
function (key, data) {
key = key.$replace ('\\', '/');
if (JW.Logger.debugging) JW.Logger.debug ("cachePut " + key);
