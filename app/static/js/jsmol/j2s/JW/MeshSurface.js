...
Clazz.load (null, "JW.MeshSurface", ["java.lang.Float", "java.util.Hashtable", "JU.AU", "$.BS", "$.List", "$.P3", "$.P4", "$.PT", "$.SB", "JW.BSUtil", "$.BoxInfo", "$.C", "$.Escape", "$.Geodesic", "$.Measure", "$.TempArray"], function () {
...
var colix = (color == this.lastColor ? this.lastColix : (this.lastColix = JW.C.getColix (this.lastColor = color)));
...
this.slabColix = JW.C.getColixTranslucent3 (JW.C.getColixS (color), true, translucency);
}, "JU.BS,JU.BS,~S,~S,~N");
$_M(c$, "getSlabColor", 
function () {
return (this.bsSlabGhost == null ? null : JW.C.getHexCode (this.slabColix));
});
$_M(c$, "getSlabTranslucency", 
function () {
return (this.bsSlabGhost == null ? null : "" + JW.C.getColixTranslucencyFractional (this.slabColix));
});
$_M(c$, "getSlabType", 
function () {
...
function () {
var isTranslucent = JW.C.isColixTranslucent (this.colix);
var translucentLevel = JW.C.getColixTranslucencyFractional (this.colix);
for (var i = 0; i < this.polygonCount; i++) if (this.bsTransPolygons.get (i)) {
if (!this.setABC (i)) continue;
this.vertexColixes[this.iA] = JW.C.getColixTranslucent3 (this.vertexColixes[this.iA], isTranslucent, translucentLevel);
this.vertexColixes[this.iB] = JW.C.getColixTranslucent3 (this.vertexColixes[this.iB], isTranslucent, translucentLevel);
this.vertexColixes[this.iC] = JW.C.getColixTranslucent3 (this.vertexColixes[this.iC], isTranslucent, translucentLevel);
}
this.bsTransPolygons = null;
this.polygonTranslucencies = null;
});
$_M(c$, "resetSlab", 
function () {
this.slabPolygons (JW.TempArray.getSlabObjectType (1048587, null, false, null), false);
});
...
return JW.TempArray.getSlabObjectType (1679429641, [JW.Escape.uP (pts[0]), JW.Escape.uP (pts[1]), JW.Escape.uP (pts[2]), JW.Escape.uP (pts[3])], isCap, null);
}var plane = JW.Escape.uP (s);
if (Clazz.instanceOf (plane, JU.P4)) return JW.TempArray.getSlabObjectType (135266319, plane, isCap, null);
} catch (e) {
...
this.bsSlabDisplay = JW.BSUtil.setAll (this.polygonCount == 0 ? this.vertexCount : this.polygonCount);
this.bsSlabGhost = null;
...
andCap = false;
this.colix = JW.C.getColixTranslucent3 (this.colix, false, 0);
}var sb =  new JU.SB ();
sb.append (andCap ? " cap " : " slab ");
if (isGhost) {
sb.append ("translucent ").appendF (JW.C.getColixTranslucencyFractional (this.slabColix)).append (" ");
var s = JW.C.getHexCode (this.slabColix);
...
sb.append (JW.Escape.eP4 (plane));
this.getIntersection (0, plane, null, null, null, null, null, andCap, false, 135266319, isGhost);
break;
case 1614417948:
case 1679429641:
var box = slabbingObject;
sb.append ("within ").append (JW.Escape.eAP (box));
var faces = JW.BoxInfo.getFacesFromCriticalPoints (box);
...
var points = o[1];
var bs = o[2];
sb.append ("within ").appendF (distance).append (bs == null ? JW.Escape.e (points) : JW.Escape.e (bs));
...
bs = (distanceMax < distance ? JW.BSUtil.copy (this.bsSlabDisplay) : null);
this.getIntersection (distance, null, null, null, null, null, null, andCap, false, 32, isGhost);
var bsA = (bs == null ? null : JW.BSUtil.copy (this.bsSlabDisplay));
JW.BSUtil.copy2 (bs, this.bsSlabDisplay);
...
if (Float.isNaN (fData[i]) || JW.MeshSurface.checkSlab (tokType, this.vertices[i], fData[i], distance, plane, ptCenters, bsSource) > 0) this.bsSlabDisplay.clear (i);
}
return;
...
d1 = JW.MeshSurface.checkSlab (tokType, vA, valA, (bsSource == null ? distance : sourceA), plane, ptCenters, bsSource);
d2 = JW.MeshSurface.checkSlab (tokType, vB, valB, (bsSource == null ? distance : sourceB), plane, ptCenters, bsSource);
d3 = JW.MeshSurface.checkSlab (tokType, vC, valC, (bsSource == null ? distance : sourceC), plane, ptCenters, bsSource);
...
case 1:
case 6:
if (ptCenters == null) pts = [JW.MeshSurface.interpolatePoint (vA, vB, -d1, d2, valA, valB, values, fracs, 0), JW.MeshSurface.interpolatePoint (vA, vC, -d1, d3, valA, valC, values, fracs, 1)];
...
case 2:
case 5:
if (ptCenters == null) pts = [JW.MeshSurface.interpolatePoint (vB, vA, -d2, d1, valB, valA, values, fracs, 1), JW.MeshSurface.interpolatePoint (vB, vC, -d2, d3, valB, valC, values, fracs, 0)];
...
case 3:
case 4:
if (ptCenters == null) pts = [JW.MeshSurface.interpolatePoint (vC, vA, -d3, d1, valC, valA, values, fracs, 0), JW.MeshSurface.interpolatePoint (vC, vB, -d3, d2, valC, valB, values, fracs, 1)];
...
}}, "~N,JU.P4,~A,JU.List,~A,JU.BS,JW.MeshSurface,~B,~B,~N,~B");
...
this.iD = JW.MeshSurface.setPoint (fracs, fD, i1, i2);
this.iE = JW.MeshSurface.setPoint (fracs, 1 - fD, i1, i3);
...
break;
case 135266319:
d = JW.Measure.distanceToPlane (plane, v);
break;
case 1276118018:
d = JW.MeshSurface.minDist (v, ptCenters) - distance;
break;
default:
d = -JW.MeshSurface.minDist (v, ptCenters) - distance;
break;
...
return JW.MeshSurface.interpolateFraction (v1, v2, JW.MeshSurface.getSphericalInterpolationFraction (absD, d1, d2, v1.distance (v2)), val1, val2, values, fracs, i);
...
return JW.MeshSurface.interpolateFraction (v1, v2, d1 / (d1 + d2), val1, val2, values, fracs, i);
...
function (lvl) {
JW.Geodesic.createGeodesic (lvl);
var vertexCount = JW.Geodesic.getVertexCount (lvl);
var f = JW.Geodesic.getFaceVertexes (lvl);
...
for (var i = 0; i < vertexCount; i++) vectors[i] = JW.Geodesic.getVertexVector (i);

return JW.MeshSurface.newMesh (true, vectors, 0, faces, vectors, 0);
}, "~N");
