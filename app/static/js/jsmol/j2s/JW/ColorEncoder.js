...
Clazz.load (null, "JW.ColorEncoder", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.AU", "$.CU", "$.List", "$.PT", "J.constant.EnumPalette", "JW.C", "$.Escape", "$.Logger", "JV.JC"], function () {
...
for (var i = 0; i < JW.ColorEncoder.colorSchemes.length; i++) if (JW.ColorEncoder.colorSchemes[i].equalsIgnoreCase (colorScheme)) return (i >= 16 ? i - 16 : i < 13 ? i : -i);
...
var ipt = JW.ColorEncoder.getSchemeIndex (name);
return (ipt >= 0 ? JW.ColorEncoder.colorSchemes[ipt] : name.toLowerCase ());
}, $fz.isPrivate = true, $fz), "~S");
$_M(c$, "makeColorScheme", 
($fz = function (name, scale, isOverloaded) {
name = JW.ColorEncoder.fixName (name);
if (scale == null) {
...
break;
case 3:
JW.ColorEncoder.getRasmolScale ();
break;
case 5:
this.argbsAmino = JV.JC.argbsAmino;
break;
...
if (c == 0) {
JW.Logger.error ("error in color value: " + colorScheme.substring (pt, pt2 + 1));
return 0;
}scale[n++] = c;
}
if (name.equals ("user")) {
...
}colorScheme = JW.ColorEncoder.fixName (colorScheme);
var ipt = JW.ColorEncoder.getSchemeIndex (colorScheme);
...
case 3:
return JW.ColorEncoder.getRasmolScale ();
case 4:
...
var colix = JW.C.getColix (this.getArgbFromPalette (val, lo, hi, palette));
if (isTranslucent) {
var f = (hi - val) / (hi - lo);
if (f > 1) f = 1;
 else if (f < 0.125) f = 0.125;
colix = JW.C.getColixTranslucent3 (colix, true, f);
}return colix;
}, "~N,~N,~N,~N,~B");
...
case 3:
return JW.ColorEncoder.getRasmolScale ().length;
case 4:
...
}return this.thisScale[JW.ColorEncoder.quantize (val, lo, hi, n)];
case 10:
return this.getPaletteBW ()[JW.ColorEncoder.quantize (val, lo, hi, n)];
case 11:
return this.getPaletteWB ()[JW.ColorEncoder.quantize (val, lo, hi, n)];
case 0:
return this.propertyColorEncoder.argbsRoygb[JW.ColorEncoder.quantize (val, lo, hi, n)];
case 1:
return this.propertyColorEncoder.argbsRoygb[JW.ColorEncoder.quantize (-val, -hi, -lo, n)];
case 8:
return this.propertyColorEncoder.argbsRoygb[JW.ColorEncoder.quantize (val, lo, hi, n)];
case 9:
return this.propertyColorEncoder.argbsRoygb[this.propertyColorEncoder.ihalf + JW.ColorEncoder.quantize (val, lo, hi, n) * 2];
case 6:
return this.propertyColorEncoder.argbsRwb[JW.ColorEncoder.quantize (val, lo, hi, n)];
case 7:
return this.propertyColorEncoder.argbsRwb[JW.ColorEncoder.quantize (-val, -hi, -lo, n)];
case -13:
return (this.propertyColorEncoder.userScale.length == 0 ? -8355712 : this.propertyColorEncoder.userScale[JW.ColorEncoder.quantize (val, lo, hi, n)]);
case -14:
return (this.propertyColorEncoder.userScale.length == 0 ? -8355712 : this.propertyColorEncoder.userScale[JW.ColorEncoder.quantize (-val, -hi, -lo, n)]);
case 2:
return this.propertyColorEncoder.argbsCpk[JW.ColorEncoder.colorIndex (val, n)];
case 3:
return JW.ColorEncoder.getRasmolScale ()[JW.ColorEncoder.colorIndex (val, n)];
case 4:
return this.propertyColorEncoder.argbsShapely[JW.ColorEncoder.colorIndex (val, n)];
case 5:
return this.propertyColorEncoder.argbsAmino[JW.ColorEncoder.colorIndex (val, n)];
case 12:
return this.getPaletteAC ()[JW.ColorEncoder.colorIndexRepeat (val, n)];
default:
return -8355712;
}
}, "~N,~N,~N,~N");
...
function (i) {
var absi = Math.abs (i);
return (i == -1 ? this.thisName : absi < JW.ColorEncoder.colorSchemes.length && absi >= 0 ? JW.ColorEncoder.colorSchemes[absi] : null);
}, "~N");
c$.getColorSchemeList = $_M(c$, "getColorSchemeList", 
function (scheme) {
if (scheme == null) return "";
var colors = "";
for (var i = 0; i < scheme.length; i++) colors += (i == 0 ? "" : " ") + JW.Escape.escapeColor (scheme[i]);

return colors;
}, "~A");
c$.getRasmolScale = $_M(c$, "getRasmolScale", 
function () {
if (JW.ColorEncoder.rasmolScale != null) return JW.ColorEncoder.rasmolScale;
JW.ColorEncoder.rasmolScale =  Clazz.newIntArray (J.constant.EnumPalette.argbsCpk.length, 0);
var argb = J.constant.EnumPalette.argbsCpkRasmol[0] | 0xFF000000;
for (var i = JW.ColorEncoder.rasmolScale.length; --i >= 0; ) JW.ColorEncoder.rasmolScale[i] = argb;

for (var i = J.constant.EnumPalette.argbsCpkRasmol.length; --i >= 0; ) {
argb = J.constant.EnumPalette.argbsCpkRasmol[i];
JW.ColorEncoder.rasmolScale[argb >> 24] = argb | 0xFF000000;
}
return JW.ColorEncoder.rasmolScale;
});
$_M(c$, "getPaletteAC", 
($fz = function () {
...
$_M(c$, "getColorScheme", 
function () {
return (this.isTranslucent ? "translucent " : "") + (this.currentPalette < 0 ? JW.ColorEncoder.getColorSchemeList (this.getColorSchemeArray (this.currentPalette)) : this.getColorSchemeName (this.currentPalette));
});
