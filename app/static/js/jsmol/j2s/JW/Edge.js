...
Clazz.load (["java.lang.Enum"], "JW.Edge", null, function () {
...
return JW.Edge.argbsHbondType[argbIndex];
}, "~N");
c$.getBondOrderNumberFromOrder = $_M(c$, "getBondOrderNumberFromOrder", 
function (order) {
order &= -131073;
if (order == 131071 || order == 65535) return "0";
if (JM.Bond.isOrderH (order) || (order & 256) != 0) return JW.Edge.EnumBondOrder.SINGLE.number;
if ((order & 224) != 0) return (order >> 5) + "." + (order & 0x1F);
return JW.Edge.EnumBondOrder.getNumberFromCode (order);
}, "~N");
c$.getCmlBondOrder = $_M(c$, "getCmlBondOrder", 
function (order) {
var sname = JW.Edge.getBondOrderNameFromOrder (order);
...
return "";
case 32768:
return JW.Edge.EnumBondOrder.STRUT.$$name;
case 1:
return JW.Edge.EnumBondOrder.SINGLE.$$name;
case 2:
return JW.Edge.EnumBondOrder.DOUBLE.$$name;
}
if ((order & 224) != 0) return "partial " + JW.Edge.getBondOrderNumberFromOrder (order);
if (JM.Bond.isOrderH (order)) return JW.Edge.EnumBondOrder.H_REGULAR.$$name;
if ((order & 256) != 0) return JW.Edge.EnumBondOrder.SINGLE.$$name;
return JW.Edge.EnumBondOrder.getNameFromCode (order);
}, "~N");
...
order &= -131073;
if ((order & 224) != 0) return JW.Edge.getPartialBondOrder (order);
...
function (name) {
return JW.Edge.EnumBondOrder.getCodeFromName (name);
}, "~S");
Clazz.pu$h ();
c$ = Clazz.decorateAsClass (function () {
this.code = 0;
...
function (a) {
for (var item, $item = 0, $$item = JW.Edge.EnumBondOrder.values (); $item < $$item.length && ((item = $$item[$item]) || true); $item++) if (item.$$name.equalsIgnoreCase (a)) return item.code;

return 131071;
}, "~S");
c$.getNameFromCode = $_M(c$, "getNameFromCode", 
function (a) {
for (var item, $item = 0, $$item = JW.Edge.EnumBondOrder.values (); $item < $$item.length && ((item = $$item[$item]) || true); $item++) if (item.code == a) return item.$$name;

return "?";
}, "~N");
c$.getNumberFromCode = $_M(c$, "getNumberFromCode", 
function (a) {
for (var item, $item = 0, $$item = JW.Edge.EnumBondOrder.values (); $item < $$item.length && ((item = $$item[$item]) || true); $item++) if (item.code == a) return item.number;
