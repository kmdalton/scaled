...
Clazz.load (null, "JW.Measure", ["java.lang.Float", "JU.List", "$.P3", "$.P4", "$.Quat", "$.V3", "J.api.Interface", "JM.Atom", "JW.Escape", "$.Logger", "JV.JC"], function () {
c$ = Clazz.declareType (JW, "Measure");
...
return JW.Measure.computeAngle (pointA, pointB, pointC, vectorBA, vectorBC, asDegrees);
}, "JU.T3,JU.T3,JU.T3,~B");
...
pt_b_prime.add (n);
theta = JW.Measure.computeTorsion (a, pt_a_prime, pt_b_prime, b, true);
if (Float.isNaN (theta) || r.length () < 0.0001) theta = dq.getThetaDirectedV (n);
if (tokType == 135266305) return Float.$valueOf (theta);
if (tokType == 135176) return "draw ID \"" + id + "\" VECTOR " + JW.Escape.eP (pt_a_prime) + " " + JW.Escape.eP (n) + " color " + (theta < 0 ? "{255.0 200.0 0.0}" : "{255.0 0.0 128.0}");
if (tokType == 1746538509) return "measure " + JW.Escape.eP (a) + JW.Escape.eP (pt_a_prime) + JW.Escape.eP (pt_b_prime) + JW.Escape.eP (b);
...
case 1073742001:
return [JW.Escape.eP (pt_a_prime), JW.Escape.eP (n), JW.Escape.eP (r), JW.Escape.eP (JU.P3.new3 (theta, pitch, residuesPerTurn))];
default:
return null;
}
...
var w = JW.Measure.getNormalThroughPoints (pointA, pointB, pointC, vNorm, vAB, vAC);
...
var nd = JW.Measure.getNormalThroughPoints (pointA, pointB, pointC, vNorm, vAB, vAC);
if (ptRef != null) {
...
JW.Measure.calcNormalizedNormal (pointA, pointB, pointC, vNorm, vAB, vAC);
vAB.setT (pointA);
return -vAB.dot (vNorm);
}, "JU.P3,JU.P3,JU.P3,JU.V3,JU.V3,JU.V3");
c$.getPlaneProjection = $_M(c$, "getPlaneProjection", 
function (pt, plane, ptProj, vNorm) {
var dist = JW.Measure.distanceToPlane (plane, pt);
...
var vAB =  new JU.V3 ();
var vAC =  new JU.V3 ();
var d = JW.Measure.getNormalThroughPoints (ptA, ptB, ptC, normal, vAB, vAC);
var isReversed = (JW.Measure.distanceToPlaneV (normal, d, ptCenter) > 0);
...
vTemp.setT (vAB);
vTemp.normalize ();
JW.Measure.getPlaneThroughPoint (ptTemp, vTemp, plane);
...
JW.Measure.calcAveragePointN (points, nPoints, axisA);
var nTries = 0;
while (nTries++ < nTriesMax && JW.Measure.findAxis (points, nPoints, axisA, axisUnitVector, vectorProjection) > 0.001) {
}
var tempA = JU.P3.newP (points[0]);
JW.Measure.projectOntoAxis (tempA, axisA, axisUnitVector, vectorProjection);
axisA.setT (tempA);
...
pt.setT (points[i]);
ptProj.setT (pt);
JW.Measure.projectOntoAxis (ptProj, axisA, axisUnitVector, vectorProjection);
...
var cptsA = JW.Measure.getCenterAndPoints (ptsA);
var cptsB = JW.Measure.getCenterAndPoints (ptsB);
var retStddev =  Clazz.newFloatArray (2, 0);
var q = JW.Measure.calculateQuaternionRotation ([cptsA, cptsB], retStddev, doReport);
...
if (Clazz.instanceOf (aij, JM.Atom) && Clazz.instanceOf (bij, JM.Atom)) JW.Logger.info (" atom 1 " + (aij).getInfo () + "\tatom 2 " + (bij).getInfo ());
 else break;
}
if (n < 2) return q;
var Sxx = 0;
...
retStddev[0] = JW.Measure.getRmsd (centerAndPoints, q);
...
retStddev[1] = JW.Measure.getRmsd (centerAndPoints, q);
return q;
}, "~A,~A,~B");
...
JW.Measure.getPlaneThroughPoints (ptC, ptD, ptA, vTemp, vTemp2, vTemp3, plane);
var b = (JW.Measure.distanceToPlane (plane, pt) >= 0);
JW.Measure.getPlaneThroughPoints (ptA, ptD, ptB, vTemp, vTemp2, vTemp3, plane);
if (b != (JW.Measure.distanceToPlane (plane, pt) >= 0)) return false;
JW.Measure.getPlaneThroughPoints (ptB, ptD, ptC, vTemp, vTemp2, vTemp3, plane);
if (b != (JW.Measure.distanceToPlane (plane, pt) >= 0)) return false;
JW.Measure.getPlaneThroughPoints (ptA, ptB, ptC, vTemp, vTemp2, vTemp3, plane);
var d = JW.Measure.distanceToPlane (plane, pt);
if (fullyEnclosed) return (b == (d >= 0));
var d1 = JW.Measure.distanceToPlane (plane, ptD);
...
JW.Measure.getPlaneProjection (pt1, plane, ptRet, tempNorm);
