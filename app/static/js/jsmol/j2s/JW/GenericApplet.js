...
Clazz.load (["J.api.JmolAppletInterface", "$.JmolStatusListener", "java.util.Hashtable"], "JW.GenericApplet", ["java.lang.Boolean", "java.net.URL", "javajs.awt.Dimension", "JU.List", "$.PT", "$.SB", "J.constant.EnumCallback", "J.i18n.GT", "JW.Logger", "JV.JC", "$.Viewer"], function () {
...
}, JW, "GenericApplet", null, [J.api.JmolAppletInterface, J.api.JmolStatusListener]);
...
this.initOptions ();
JW.GenericApplet.checkIn (this.fullName, this.appletObject);
...
this.viewer =  new JV.Viewer (this.viewerOptions);
...
JW.Logger.warn ("Note -- Presence of message callback disables disable translation; to enable message translation use jmolSetTranslation(true) prior to jmolApplet()");
}if (this.doTranslate) JW.Logger.warn ("Note -- Automatic language translation may affect parsing of message callbacks messages; use scriptCallback or errorCallback to process errors");
}if (!this.doTranslate) {
J.i18n.GT.setDoTranslate (false);
JW.Logger.warn ("Note -- language translation disabled");
...
this.viewer = null;
JW.GenericApplet.checkOut (this.fullName);
});
...
$_V(c$, "register", 
function (id, jsi) {
JW.GenericApplet.checkIn (id, jsi);
...
if (!this.haveNotifiedError) if (JW.Logger.debugging) {
JW.Logger.debug (type.name () + "Callback call error to " + callback + ": " + e);
...
JW.GenericApplet.findApplets (appletName, this.syncId, this.fullName, apps);
var nApplets = apps.size ();
if (nApplets == 0) {
if (!doCallback && !appletName.equals ("*")) JW.Logger.error (this.fullName + " couldn't find applet " + appletName);
return "";
...
var app = JW.GenericApplet.htRegistry.get (theApplet);
var isScriptable = true;
if (JW.Logger.debugging) JW.Logger.debug (this.fullName + " sending to " + theApplet + ": " + script);
try {
...
if (Clazz.exceptionOf (e, Exception)) {
if (!this.haveNotifiedError) if (JW.Logger.debugging) {
JW.Logger.debug ("syncCallback call error to " + syncCallback + ": " + e);
}this.haveNotifiedError = true;
} else {
throw e;
}
...
function () {
JW.GenericApplet.checkIn (null, null);
return JW.GenericApplet.htRegistry;
});
$_V(c$, "showUrl", 
function (urlString) {
if (JW.Logger.debugging) JW.Logger.debug ("showUrl(" + urlString + ")");
...
if (name != null) {
JW.Logger.info ("AppletRegistry.checkIn(" + name + ")");
JW.GenericApplet.htRegistry.put (name, applet);
}if (JW.Logger.debugging) {
for (var entry, $entry = JW.GenericApplet.htRegistry.entrySet ().iterator (); $entry.hasNext () && ((entry = $entry.next ()) || true);) {
var theApplet = entry.getKey ();
JW.Logger.debug (theApplet + " " + entry.getValue ());
}
}}, "~S,~O");
c$.checkOut = $_M(c$, "checkOut", 
function (name) {
JW.GenericApplet.htRegistry.remove (name);
}, "~S");
...
for (var i = 0; i < names.length; i++) JW.GenericApplet.findApplets (names[i], mySyncId, excludeName, apps);

return;
}var ext = "__" + mySyncId + "__";
if (appletName == null || appletName.equals ("*") || appletName.equals (">")) {
for (var appletName2, $appletName2 = JW.GenericApplet.htRegistry.keySet ().iterator (); $appletName2.hasNext () && ((appletName2 = $appletName2.next ()) || true);) {
...
if (!JW.GenericApplet.htRegistry.containsKey (appletName)) appletName = "jmolApplet" + appletName;
if (!appletName.equals (excludeName) && JW.GenericApplet.htRegistry.containsKey (appletName)) {
