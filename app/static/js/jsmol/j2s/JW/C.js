...
Clazz.load (["JW.Int2IntHash"], "JW.C", ["java.lang.Byte", "$.Float", "JU.AU", "$.CU", "$.PT", "$.SB", "J.constant.EnumPalette", "JW.Escape", "$.Logger"], function () {
c$ = Clazz.declareType (JW, "C");
...
translucentFlag = JW.C.getTranslucentFlag ((argb >> 24) & 0xFF);
argb |= 0xFF000000;
}var c = JW.C.colixHash.get (argb);
if ((c & 3) == 3) translucentFlag = 0;
return ((c > 0 ? c : JW.C.allocateColix (argb)) | translucentFlag);
}, "~N");
c$.allocateColix = $_M(c$, "allocateColix", 
function (argb) {
for (var i = JW.C.colixMax; --i >= 4; ) if ((argb & 0xFFFFFF) == (JW.C.argbs[i] & 0xFFFFFF)) return i;

if (JW.C.colixMax == JW.C.argbs.length) {
var oldSize = JW.C.colixMax;
var newSize = oldSize * 2;
if (newSize > 2048) newSize = 2048;
JW.C.argbs = JU.AU.arrayCopyI (JW.C.argbs, newSize);
if (JW.C.argbsGreyscale != null) JW.C.argbsGreyscale = JU.AU.arrayCopyI (JW.C.argbsGreyscale, newSize);
}JW.C.argbs[JW.C.colixMax] = argb;
if (JW.C.argbsGreyscale != null) JW.C.argbsGreyscale[JW.C.colixMax] = JU.CU.toFFGGGfromRGB (argb);
JW.C.colixHash.put (argb, JW.C.colixMax);
return (JW.C.colixMax < 2047 ? JW.C.colixMax++ : JW.C.colixMax);
}, "~N");
c$.setLastGrey = $_M(c$, "setLastGrey", 
function (argb) {
JW.C.calcArgbsGreyscale ();
JW.C.argbsGreyscale[2047] = JU.CU.toFFGGGfromRGB (argb);
}, "~N");
c$.calcArgbsGreyscale = $_M(c$, "calcArgbsGreyscale", 
function () {
if (JW.C.argbsGreyscale != null) return;
var a =  Clazz.newIntArray (JW.C.argbs.length, 0);
for (var i = JW.C.argbs.length; --i >= 4; ) a[i] = JU.CU.toFFGGGfromRGB (JW.C.argbs[i]);

JW.C.argbsGreyscale = a;
});
c$.getArgbGreyscale = $_M(c$, "getArgbGreyscale", 
function (colix) {
if (JW.C.argbsGreyscale == null) JW.C.calcArgbsGreyscale ();
return JW.C.argbsGreyscale[colix & -30721];
}, "~N");
c$.getColixO = $_M(c$, "getColixO", 
function (obj) {
if (obj == null) return 0;
if (Clazz.instanceOf (obj, J.constant.EnumPalette)) return ((obj) === J.constant.EnumPalette.NONE ? 0 : 2);
if (Clazz.instanceOf (obj, Integer)) return JW.C.getColix ((obj).intValue ());
if (Clazz.instanceOf (obj, String)) return JW.C.getColixS (obj);
if (Clazz.instanceOf (obj, Byte)) return ((obj).byteValue () == 0 ? 0 : 2);
if (JW.Logger.debugging) {
JW.Logger.debug ("?? getColix(" + obj + ")");
}return 22;
}, "~O");
...
colix &= -30721;
if (colix == 0) colix = 1;
return (isTranslucent ? (colix | JW.C.getTranslucentFlag (translucentLevel)) : colix);
}, "~N,~B,~N");
c$.copyColixTranslucency = $_M(c$, "copyColixTranslucency", 
function (colixFrom, colixTo) {
return JW.C.getColixTranslucent3 (colixTo, JW.C.isColixTranslucent (colixFrom), JW.C.getColixTranslucencyLevel (colixFrom));
}, "~N,~N");
c$.getColixTranslucencyFractional = $_M(c$, "getColixTranslucencyFractional", 
function (colix) {
var translevel = JW.C.getColixTranslucencyLevel (colix);
...
if (argb != 0) return JW.C.getColix (argb);
...
colixes[j] = JW.C.getColix (JU.CU.getArgbFromString (colors[j]));
...
function (colix) {
return JW.Escape.escapeColor (JW.C.getArgb (colix));
}, "~N");
c$.getHexCodes = $_M(c$, "getHexCodes", 
function (colixes) {
if (colixes == null) return null;
var s =  new JU.SB ();
for (var i = 0; i < colixes.length; i++) s.append (i == 0 ? "" : " ").append (JW.C.getHexCode (colixes[i]));
...
function (argb) {
var a = (argb >> 24) & 0xFF;
return (a == 0xFF ? JW.C.getColix (argb) : JW.C.getColixTranslucent3 (JW.C.getColix (argb), true, a / 255));
}, "~N");
...
"argbsGreyscale", null);
c$.colixHash = c$.prototype.colixHash =  new JW.Int2IntHash (256);
Clazz.defineStatics (c$,
"RAW_RGB_INT", 3,
...
{
for (var i = 0; i < JW.C.predefinedArgbs.length; ++i) JW.C.getColix (JW.C.predefinedArgbs[i]);
