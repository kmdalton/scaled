...
Clazz.load (["JU.P3", "$.P3i", "$.V3"], "JW.BoxInfo", ["JU.List", "$.P4", "JW.Measure", "$.Point3fi"], function () {
...
{
for (var i = 8; --i >= 0; ) this.bbVertices[i] =  new JW.Point3fi ();

}{
for (var i = 0; i < 8; i++) {
JW.BoxInfo.unitBboxPoints[i] = JU.P3.new3 (-1, -1, -1);
JW.BoxInfo.unitBboxPoints[i].scaleAdd2 (2, JW.BoxInfo.unitCubePoints[i], JW.BoxInfo.unitBboxPoints[i]);
}
}});
...
function (scale) {
return JW.BoxInfo.getCanonicalCopy (this.bbVertices, scale);
}, "~N");
c$.getCanonicalCopy = $_M(c$, "getCanonicalCopy", 
function (bbUcPoints, scale) {
var pts =  new Array (8);
for (var i = 0; i < 8; i++) pts[JW.BoxInfo.toCanonical[i]] = JU.P3.newP (bbUcPoints[i]);

JW.BoxInfo.scaleBox (pts, scale);
return pts;
}, "~A,~N");
...
for (var i = 0; i < 6; i++) {
va.setT (vertices[JW.BoxInfo.facePoints[i].x]);
vb.setT (vertices[JW.BoxInfo.facePoints[i].y]);
vc.setT (vertices[JW.BoxInfo.facePoints[i].z]);
JW.Measure.getPlaneThroughPoints (va, vb, vc, vNorm, vAB, vAC, faces[i] =  new JU.P4 ());
}
return faces;
}, "~A");
...
function (pt) {
this.isScaleSet = false;
JW.BoxInfo.addPoint (pt, this.bbCorner0, this.bbCorner1, this.margin);
}, "JU.P3");
...
pt.setT (JW.BoxInfo.unitBboxPoints[i]);
